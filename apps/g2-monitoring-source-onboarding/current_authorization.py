...
from onboarding import CurrentAuthorizationEvidence, MANAGE_ACTION, READ_ACTION, SYNC_READ_ACTION


_ALLOWED_ACTIONS = frozenset({MANAGE_ACTION, READ_ACTION, SYNC_READ_ACTION})


class BoundMonitoringAuthorization:
    """Per-request G2 adapter over the accepted current-authority owner."""
    ...
    def require(
        ...
        *,
        ...
        if action not in _ALLOWED_ACTIONS:
            raise AdmissionDenied("action is outside the bounded G2 authorization surface")
        ...
        if len(captured) != 1:
            raise AdmissionDenied("final monitoring admission evidence is unavailable")
        evidence = captured[0]
        if evidence.tenant_id != tenant_id or evidence.action != action:
            raise AdmissionDenied("final monitoring admission evidence does not match the requested operation")
