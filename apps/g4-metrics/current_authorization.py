...
class BoundMetricsAuthorization:
    def __init__(
        self,
        *,
        principal: Principal,
    ...
    def require(
        ...
        *,
        ...
        if action not in {RESOURCE_READ_ACTION, METRIC_READ_ACTION}:
            raise AdmissionDenied("action is outside the bounded G4 authorization surface")
        ...
        if len(captured) != 1:
            raise AdmissionDenied("final metrics admission evidence is unavailable")
        evidence = captured[0]
        if evidence.tenant_id != tenant_id or evidence.action != action:
            raise AdmissionDenied("final metrics admission evidence does not match the requested operation")
