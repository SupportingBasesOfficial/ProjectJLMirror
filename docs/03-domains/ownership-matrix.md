...
| Context | Owns mutable state | May consume | Must not directly mutate |
|---|---|---|---|
...
| Monitoring | sources, devices/resources, external refs, metric definitions, current state, normalized problems, sync state | provider input | ITSM, alerts, AIOps findings |
| Alerting | alerts, notification policy/state, escalation/subscriptions | Monitoring/Infrastructure facts | Monitoring resources, ITSM tickets |
...
| Infrastructure | assets/topology, discovery, posture/capacity/drift/patch state | Monitoring/provider evidence | Monitoring current state |
...
| Compliance & Governance | controls/evidence/governance workflow state | audit and domain evidence | direct remediation of owned state |

## Ownership rules

1. Every mutable aggregate has one logical owner.
...
5. If a context is extracted into a service, the ownership model should remain valid without redefining business semantics.