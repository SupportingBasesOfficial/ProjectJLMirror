...
| Artifact delivery-lease admission race | Pause a valid capability redemption after it observes releasable state and delivery generation D but before active-lease persistence, then race governed erasure. Lease creation and delivery-generation retirement use one logical serialization authority: if lease admission commits first it is durably included in the erasure abort/drain set; if erasure commits first the stale D lease admission fails, no stream is created and zero protected bytes are released |
...
- an active artifact delivery lease can be admitted for a retired delivery generation after erasure has closed admission/reconciled older leases, allowing a stream or first protected byte to start because lease creation was check-then-insert rather than serialized with the fence;
