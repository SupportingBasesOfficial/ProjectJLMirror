...
| Security/session acceleration cache unavailable, durable authority reachable | Bypass to the durable session/permission authority under an explicit per-cell bulkhead/concurrency budget sized for full-cache-miss load; do not fail closed platform-wide on this condition alone | Security-sensitive mutations continue via the durable authority; performance degrades, not correctness | Restore cache under budget; alert on sustained bypass load; this row never substitutes for the row above when the durable authority itself is unavailable |
