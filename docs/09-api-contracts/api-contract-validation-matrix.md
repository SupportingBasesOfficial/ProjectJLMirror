...
| Realtime subscription authorization freshness | Protected subscription creation re-establishes current tenant/resource authorization; active subscriptions use governed bounded revalidation and/or equivalent active invalidation so connection-time authority cannot remain frozen indefinitely |
| Realtime subscription active invalidation | Session revocation, membership disable/revocation, permission/scope removal and tenant suspension/access denial retire affected protected delivery within the accepted security bound; transport details remain OPEN |
| Realtime subscription placement retirement | Retiring a tenant placement/admission generation retires affected subscription/connection authority; a still-open source socket cannot remain authoritative and protected delivery resumes only after current routing/authorization plus resubscribe/resync |
...
| Breaking-change CI | Contract diff detects structural risk; semantic review checks HTTP canonicalization/structured-entity/idempotency-recovery-continuity/response-header/security/ownership/retry/consistency/cache/callback-freshness-replay-recovery/realtime-admission-subscription-lifecycle/browser-delivery/cursor/parser changes |
...
- a `realtime-admission` endpoint lacks governed manifest metadata for ticket scope/expiry, current authority/placement, atomic single-winner consume, burn-on-ambiguity, replay recovery continuity or subscription separation/freshness/invalidation/placement retirement;
...
- a supposedly compatible same-major change removes/renames/reinterprets accepted behavior or changes safe retry/security/consistency/HTTP-framing/structured-body/idempotency-recovery-continuity/response-header/path-query/cache/callback-freshness-replay-recovery/realtime-admission-subscription-lifecycle/browser-delivery/cursor/archive/parser semantics;
