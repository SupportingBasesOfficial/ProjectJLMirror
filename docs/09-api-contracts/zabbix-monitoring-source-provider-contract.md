...
- normal incremental polling re-reads an accepted bounded overlap behind the provisional high-water mark so late-arriving older values can still be discovered idempotently;
- provider/proxy outage, source recovery, detected gap or another condition that may exceed the ordinary overlap triggers a wider bounded historical reconciliation/backfill window rather than pretending the previous high-water mark is complete;
- a historical region becomes **finalized only under an explicit accepted lateness/reconciliation horizon** supported by provider-retention/capacity evidence. Finalization means "complete under the accepted provider lateness contract," not a claim that event time is a globally monotonic append log.

If a bounded request reaches its configured limit while additional same-second values cannot be ruled out, or if lateness exceeds the currently provable reconciliation horizon, the adapter SHALL NOT silently advance/declare complete. It enters a visible degraded/reconciliation/capacity state, preserves the last safe checkpoint/floor, and retries or widens the bounded recovery window according to the accepted provider profile. If retained provider history can no longer prove the missing interval, JLMirror records an explicit history-gap/incomplete condition; it does not fabricate completeness.

Exact overlap/lateness/backfill horizons and row/window/body limits remain evidence-driven OPEN numerics. The fixed property is stronger: no strict event-time cursor, page truncation or delayed provider insertion may silently manufacture a false "fully synchronized" history state.
...
- a history value that becomes query-visible later with an older `clock` (for example after buffered provider/proxy delivery) is discovered by the bounded overlap/reconciliation path and deduplicated canonically, rather than being lost behind a strict forward event-time cursor;
- a provider/proxy outage that can exceed the ordinary overlap causes a wider bounded reconciliation/backfill or an explicit incomplete/gap state; it does not preserve a false fully-synchronized watermark.
...
- Exact `history.get` row/window/body bounds, normal overlap, accepted lateness/finalization horizon and outage/backfill widening policy: `OPEN`, pending provider-retention/capacity evidence; silent checkpoint advancement or false completeness is never an accepted fallback.
