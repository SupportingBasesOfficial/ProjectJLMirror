...
- `history.get` — the actual `metric_observation` history stream, retrieved per item/value type under the bounded complete-window/checkpoint rules below; Zabbix's float/unsigned/string/text/log history classes are queried explicitly because one call does not unify value types;
...
`item.get` supplies the provider's latest/current snapshot cheaply on every applicable cycle; current-state authority from that snapshot is ordered by the platform-owned fenced poll generation defined below, **not** by `lastclock`/`lastns`. `history.get` is what supplies the complete historical `metric_observation` stream required by `FR-MON-005`; historical/backfill observations remain valid history but do not gain current-state authority merely because their provider event time is numerically greater than another sample's timestamp.
...
### Current-state ordering authority: fenced poll generation, not provider wall clock
...
Current/latest snapshot projections therefore use a platform-owned, durable `zabbix_poll_generation` scoped to `(tenant_id, monitoring_source_id, zabbix_instance_generation)`. Every authoritative poll cycle that may mutate current state SHALL first acquire a durable single-winner poll claim/fence and its next monotonic poll generation. Scheduled and webhook-triggered cycles use the **same** authority. A cycle may publish current-state candidates only while it still proves that exact claim/generation current; a predecessor whose lease/fence was lost, superseded or recovered from stale state cannot commit even if its network response arrives later. The canonical current-state projection ordering token is therefore conceptually:

```text
(zabbix_instance_generation, zabbix_poll_generation)
```

The provider's `lastclock`/`lastns`, problem/event timestamps and similar fields remain source-time metadata for freshness/history/diagnostics. They MUST NOT override the fenced poll generation or resurrect a stale poll. A Zabbix wall-clock rollback inside the same instance consequently cannot freeze current state until the clock catches up, and an old delayed poll cannot win merely because it finishes later.

A `metric_observation` may already have been durably accepted through `history.get` before the same sample is later seen as `item.get`'s current/latest value. That prior historical acceptance does not block the current-state projection attempt: the current-state candidate is evaluated independently under the fenced poll-generation token while historical dispatch remains idempotent by canonical observation identity. Conversely, a historical/backfill observation that is not a current-snapshot candidate does not advance current state on `clock`/`ns` alone.

### `history.get` incremental completeness and checkpoint safety
...
For each `(tenant, monitoring_source, zabbix_instance_generation, itemid, history_value_type)` history stream, the adapter SHALL use bounded windows with deterministic `clock`/`ns` ordering, inclusive boundary overlap and canonical observation deduplication. The durable history checkpoint records enough boundary state to prove which second/nanosecond region has been fully drained. A checkpoint advances beyond a timestamp second **only after completeness of that boundary has been proven** under the accepted row/body limits.

If a bounded request reaches its configured limit while additional same-second values cannot be ruled out with the Zabbix API primitives, the adapter SHALL NOT guess that the boundary is complete and SHALL NOT advance the checkpoint. It enters a visible degraded/reconciliation/capacity state and retries under the accepted bounded recovery strategy (for example a narrower per-item/time window or an evidenced higher bound) until completeness is proven. Silent sample loss is prohibited; an unpageable saturated boundary is an explicit operational condition, not permission to skip data.
...
untrusted webhook POST
  ...
  -> the actual poll must acquire the same durable per-source poll claim/fence and poll generation
...
- a Zabbix clock rollback inside one unchanged `zabbix_instance_generation` does not freeze a genuinely newer current snapshot: the later valid fenced poll generation may advance current state even when its `lastclock`/`lastns` is numerically lower;
...
- a `history.get` boundary containing more same-second values than one configured page can prove complete does not advance the durable history checkpoint or silently lose rows; it becomes a visible bounded degraded/reconciliation condition until completeness is established.
...
- Exact `history.get` row/window/body bounds and the numeric threshold at which same-second saturation enters the bounded recovery/degraded path: `OPEN`, pending provider/capacity evidence; silent checkpoint advancement is never an accepted fallback.
