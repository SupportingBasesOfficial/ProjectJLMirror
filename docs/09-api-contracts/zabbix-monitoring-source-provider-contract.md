...
- `hostgroup.get` — validates the configured host-group scope anchors and the integration user's current visibility of those anchors; it is not tenant authority and is not used to broaden configured scope;
...
- `event.get` — historical/recovery event stream, including object-specific reconciliation where required, bounded by stable event identity/time-range rules.
...
`zabbix_poll_epoch` is an ordered **platform authority epoch**, not a counter whose freshness may be trusted merely because it was loaded from the same database image being recovered. `zabbix_poll_generation` is monotonic inside one current epoch. Every authoritative poll cycle that may mutate current state SHALL first acquire a durable single-winner poll claim/fence for the current epoch and its next generation. Scheduled and webhook-triggered cycles use the **same** authority. A cycle may publish current-state candidates only while it proves that exact epoch/generation, the corresponding current tenant placement/admission **and that its `zabbix_instance_generation` still equals the source's authoritative `active_source_instance_generation`**. A predecessor whose lease/fence, placement, poll epoch or source-instance generation authority was lost cannot commit current state even if its network response arrives later.
...
A `metric_observation` may already have been durably accepted through `history.get` before the same sample is later seen as `item.get`'s current/latest value. That prior historical acceptance does not block the current-state projection attempt: the current-state candidate is evaluated independently under the current fenced poll epoch/generation while historical dispatch remains idempotent by canonical observation identity. Conversely, a historical/backfill observation that is not a current-snapshot candidate does not advance current state on `clock`/`ns` alone. After source-instance cutover, replay/backfill from the retired generation may repair retained history under its generation-scoped identity but cannot regain current-state authority.
...
### Snapshot completeness, provider visibility and negative evidence

`host.get`, `item.get`, `trigger.get` and `problem.get` can be consumed as bounded/partitioned snapshots, but **omission is not authoritative absence unless both provider visibility and the relevant coverage are proven current/complete**. This materializes the repository-wide `uncertainty != absence` rule at the provider boundary.

Before snapshot omission can authorize a negative transition, the adapter validates the configured scope anchors through `hostgroup.get` under the same authenticated integration identity. Every configured host-group reference that the source contract says is active must remain resolvable/visible. If a configured anchor disappears, becomes inaccessible, or the provider cannot prove the integration user's expected read visibility, the source enters an explicit configuration/visibility-degraded state. JLMIRROR does **not** reinterpret the resulting shrinkage as mass host/item/trigger/problem removal. Deleting a configured provider-side group or externally revoking the integration user's visibility is therefore an operational/configuration reconciliation event, not implicit business-state absence.

Every snapshot-like endpoint execution is bound to the current fenced poll epoch/generation plus an endpoint/scope identity. For current-state/negative-inference authority it is additionally bound to the source's currently active instance generation. The adapter records which partitions/pages/scopes completed and whether any request was truncated, failed, timed out, rejected or became stale. Positive historical observations may be accepted idempotently as bounded partitions arrive when their own contract permits it, but a negative provider transition such as:

- monitoring resource removed;
...
SHALL NOT be inferred from a missing row in an incomplete, truncated, failed, stale, visibility-degraded, retired-generation or scope-unreconciled snapshot.

A JLMIRROR `configured_provider_scope` edit is **not** one of these provider negative transitions. Scope inclusion/exclusion is governed separately by the Monitoring domain/API `scope_revision` contract: exclusion may produce `scope_state=out_of_scope`, but it does not by itself prove provider removal, metric retirement or problem resolution.

A negative transition becomes eligible only through one of these evidence classes:

1. current configured provider-scope visibility is proven, the adapter proves complete coverage of the exact scope under the current poll and active-instance-generation authority, and the provider contract makes omission authoritative for that object class; or
2. current configured provider-scope visibility is proven and the adapter performs a bounded authenticated object-specific reconciliation using the stable provider identity plus configured scope, with `event.get`/another applicable read used where the object class needs explicit recovery/closure evidence rather than simple absence; or
3. another explicitly accepted provider primitive provides stronger deletion/recovery/closure evidence.

For large mutable collections, complete coverage does not require one enormous transaction or response. The adapter may stage/mark `last_seen` by poll token and reconcile partitions incrementally, but **the durable snapshot-complete marker is itself authoritative evidence** and is written only after every required partition has completed under compatible current poll, placement, active-instance-generation and configured-scope authority and the configured scope anchors remained current. Missing rows become negative candidates only after that marker exists; where the provider cannot guarantee a transactionally stable multi-page snapshot, candidates requiring destructive/resolve semantics are revalidated individually before the negative transition commits. A later page failure, visibility-anchor failure, scope revision, source-instance cutover or authority loss invalidates snapshot-complete eligibility for that run rather than converting unvisited/invisible objects into absence.

Current-state transitions caused by negative evidence use the same poll epoch/generation and atomic transition/signal-intent rule as positive current-state advancement. Recovery/relocation cannot restore an old snapshot-complete/visibility marker as current authority without the same active-instance-generation/epoch/placement continuity proof required for polling generally.
...
- response body size limit: bounded, sized to the largest legitimate `hostgroup.get`/`host.get`/`item.get`/`trigger.get`/`problem.get`/`event.get`/`history.get` page for the configured pagination/window strategy;
...
- after source-instance cutover, a delayed/retired-generation poll cannot mutate current resource/metric/problem/health state even if its own old poll claim would otherwise look locally valid; only historical replay explicitly authorized for that generation may continue under historical semantics;
...
- a configured host-group anchor becoming invisible/inaccessible cannot be interpreted as all member hosts/items/triggers/problems disappearing; the integration enters visibility/configuration degradation and blocks negative inference;
...
- Exact bounded partitioning/coverage strategy, configured-scope visibility verification cadence and object-specific negative-revalidation thresholds for snapshot-like `host.get`/`item.get`/`trigger.get`/`problem.get`: `OPEN`, pending provider/capacity evidence; incomplete/uncertain visibility never authorizes negative inference.
