...
| `rel.external-provider@1` | `unavailable`, `slow_or_timed_out`, `throttled`, `compromised_or_untrusted` | `unavailable→fail_fast`; `slow_or_timed_out→fail_fast`; `throttled→queued_or_deferred` only for an already Product-authorized durable path, otherwise `fail_fast`; `compromised_or_untrusted→capability_unavailable` | local operation ledger plus authoritative provider inquiry under current destination generation |
