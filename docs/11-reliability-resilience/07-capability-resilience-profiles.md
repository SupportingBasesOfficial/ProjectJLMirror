...
| `RP-OPTIONAL-TRANSPORT` | optional same-record/source identity only while data-class policy and capacity admit | attempts, elapsed time, concurrency, buffered count, buffered bytes, cardinality and attributable cost use `OPEN-REL-006`; speculation=`no_applicable_case`; redrive=`no_applicable_case` unless a later accepted data-class profile authorizes it; restart preserves accepted-buffer budget | pressure-reducing delay before explicit shedding; numeric curve/cap=`OPEN-REL-006` | decorrelated by tenant/source/data class; distribution=`OPEN-REL-006`; synchronized refill forbidden | `shed_or_reject` with explicit loss; cannot block business truth or claim completeness |
