...
| `rel.placement-reference-cache` | `1` | Platform Management; tenant/cell/generation | bounded signed/versioned copy; authority: Control Plane plus destination admission; optional: cache transport | unavailable with verified unexpired copy and destination admission → `stale_tolerant`, otherwise `fail_fast`; stale/identity conflict → `fail_closed` | tenant/generation key and fallback concurrency; resume/refill after freshness and admission proof | `FV-CP-002`, `FV-CACHE-001`; `RB-REL-001`, `RB-REL-005`; `OPEN-REL-001`, `OPEN-REL-002`, `OPEN-REL-015` |
...
| `rel.placement-reference-cache` | `1` | `unavailable[placement_cache_fallback_state=verified_unexpired_copy_and_destination_admitted]:stale_tolerant`; `unavailable[placement_cache_fallback_state=fallback_ineligible]:fail_fast`; `stale:fail_closed`; `identity_conflict:fail_closed` |
