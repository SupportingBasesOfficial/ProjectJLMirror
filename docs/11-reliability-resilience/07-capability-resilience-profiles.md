...
|---|---|
| every `rel.*` profile and every version | `FV-RETRY-001`, `FV-RETRY-002`, `FV-RETRY-003`, `FV-COMP-001`, `FV-EVID-001`, `FV-AI-001`; `RB-REL-021`, `RB-REL-022`, `RB-REL-024`, `RB-REL-025`; `OPEN-REL-005`, `OPEN-REL-006`, `OPEN-REL-007`, `OPEN-REL-021`, `OPEN-REL-022`, `OPEN-REL-023`, `OPEN-REL-026`, `OPEN-REL-028` |

The selector is normative and mechanically expands to every catalog key; it is not an optional inheritance rule. A profile version cannot opt out of deadline, retry, compatibility or evidence-integrity coverage. Circuit evidence is derived separately from the exact profile selector below so that a declared `no_applicable_case` does not inherit an impossible half-open experiment.
...
Every referenced vector and blocker SHALL exist, and each vector SHALL resolve to exactly one blocker row in `09-reliability-validation-and-fault-matrix.md`. `CIRCUIT-EVIDENCE` SHALL take exactly one branch. A `no_applicable_case` selector cannot import applicable-circuit probe/open vectors and SHALL provide a dedicated negative-evidence branch. Static conformance SHALL reject any profile for which an element of `ALL-FAULT-VECTORS` is absent from evidence or its vector-derived blocker is absent from `ALL-RELEASE-BLOCKERS`.
