...
|---|---|---|---|---|---|---|
| `rel.control-plane-placement` | `1` | Platform Management; tenant placement/cell lifecycle | Control Plane placement/lifecycle generation; hard: authority store; continuity: deny, placement and recovery generations | unavailable/partitioned → `stale_tolerant` only with a verified unexpired lease and destination admission, otherwise `fail_closed`; stale/identity conflict → `fail_closed`; recovery blocked → `reconciliation_blocked` | Control Plane vs cell and tenant lifecycle; resume after current generation, destination admission and `(R,F]` reconciliation | `FV-CP-001`, `FV-CP-002`, `FV-REL-001`; `RB-REL-001`, `RB-REL-019`; `OPEN-REL-001`, `OPEN-REL-002`, `OPEN-REL-004` |
...
| `rel.security-session-authority` | `1` | Security authority; principal/tenant/scope | current authn/authz/revocation/deny generation; hard: verification authority; continuity: revocation and audit | unavailable/stale/compromised → `fail_closed`; policy denied → `capability_unavailable` | principal, tenant and scope; resume only after current non-regressing deny/revocation evidence | `FV-SEC-001`; `RB-REL-004`; `OPEN-REL-026`, `OPEN-REL-027`, `OPEN-REL-031` |
| `rel.placement-reference-cache` | `1` | Platform Management; tenant/cell/generation | bounded signed/versioned copy; authority: Control Plane plus destination admission; optional: cache transport | unavailable with verified unexpired copy and destination admission → `stale_tolerant`, otherwise `fail_closed`; stale/identity conflict → `fail_closed` | tenant/generation key and fallback concurrency; resume/refill after freshness and admission proof | `FV-CP-002`, `FV-CACHE-001`; `RB-REL-001`, `RB-REL-005`; `OPEN-REL-001`, `OPEN-REL-002`, `OPEN-REL-015` |
| `rel.performance-cache` | `1` | Owning application capability; tenant/operation | authoritative origin; optional: derived cache | unavailable → `fail_fast` to origin when bounded; stale → `stale_tolerant` only under an accepted Product/freshness contract, otherwise `fail_fast`; saturated → `shed_or_reject` | tenant/operation refill bulkhead; resume after origin health and stampede control | `FV-CACHE-001`, `FV-OVER-001`; `RB-REL-005`, `RB-REL-012`; `OPEN-REL-007`, `OPEN-REL-008`, `OPEN-REL-015` |
...
| `rel.external-provider` | `1` | Owning application process plus provider adapter; tenant/integration/provider/destination | stable platform operation and authoritative provider inquiry/result; hard: local operation ledger; external: provider | unavailable/slow → `fail_fast`; throttled → bounded `queued_or_deferred` only on a Product-authorized durable path, otherwise `fail_fast`; ambiguous → `reconciliation_blocked`; compromised/permanent → `capability_unavailable` | tenant/integration/provider/destination concurrency and circuit; resume after provider/local outcome reconciliation and gradual probes | `FV-EXT-001`, `FV-EXT-002`; `RB-REL-007`, `RB-REL-008`; `OPEN-REL-005`, `OPEN-REL-006`, `OPEN-REL-007`, `OPEN-REL-008`, `OPEN-REL-011`, `OPEN-REL-014` |
...
|---|---|---|
| `rel.control-plane-placement` | `1` | `unavailable[placement_fallback_state=verified_unexpired_lease_and_destination_admitted]:stale_tolerant`; `unavailable[placement_fallback_state=fallback_ineligible]:fail_closed`; `partitioned[placement_fallback_state=verified_unexpired_lease_and_destination_admitted]:stale_tolerant`; `partitioned[placement_fallback_state=fallback_ineligible]:fail_closed`; `stale:fail_closed`; `identity_conflict:fail_closed`; `recovery_continuity_blocked:reconciliation_blocked` |
...
| `rel.placement-reference-cache` | `1` | `unavailable[placement_cache_fallback_state=verified_unexpired_copy_and_destination_admitted]:stale_tolerant`; `unavailable[placement_cache_fallback_state=fallback_ineligible]:fail_closed`; `stale:fail_closed`; `identity_conflict:fail_closed` |
| `rel.performance-cache` | `1` | `unavailable:fail_fast`; `stale[stale_read_state=product_authorized_within_freshness_contract]:stale_tolerant`; `stale[stale_read_state=not_authorized_or_outside_contract]:fail_fast`; `saturated:shed_or_reject` |
...
| `rel.configuration-authority` | `1` | `unavailable[configuration_fallback_state=verified_permitted_last_known_good]:stale_tolerant`; `unavailable[configuration_fallback_state=fallback_ineligible]:fail_closed`; `contract_permanent:fail_closed`; `identity_conflict:fail_closed`; `out_of_order_or_gap:fail_closed`; `stale:fail_closed`; `compromised_or_untrusted:fail_closed`; `recovery_continuity_blocked:reconciliation_blocked` |
| `rel.outbox-publication` | `1` | `unavailable:queued_or_deferred`; `throttled:queued_or_deferred`; `identity_conflict:reconciliation_blocked`; `saturated[intent_state=not_committed]:shed_or_reject`; `saturated[intent_state=committed]:queued_or_deferred` |
...
| `rel.external-provider` | `1` | `unavailable:fail_fast`; `slow_or_timed_out:fail_fast`; `throttled[provider_path_state=product_authorized_durable_path]:queued_or_deferred`; `throttled[provider_path_state=no_authorized_durable_path]:fail_fast`; `external_outcome_ambiguous:reconciliation_blocked`; `compromised_or_untrusted:capability_unavailable`; `contract_permanent:capability_unavailable` |
...
| `rel.customer-telemetry-acceptance` | `1` | `unavailable:queued_or_deferred`; `saturated[acceptance_state=not_durably_accepted]:shed_or_reject`; `saturated[acceptance_state=durably_accepted]:queued_or_deferred`; `duplicate:fail_fast` through canonical scoped dedup; `identity_conflict:reconciliation_blocked`; `out_of_order_or_gap:reconciliation_blocked` for affected current-state/signal projection while historical acceptance remains durable; `recovery_continuity_blocked:reconciliation_blocked` |
...
| `rel.reporting-derived` | `1` | `unavailable:queued_or_deferred`; `slow_or_timed_out:queued_or_deferred`; `saturated:shed_or_reject`; `stale[stale_result_state=product_authorized_within_disclosure_contract]:stale_tolerant`; `stale[stale_result_state=not_authorized_or_outside_contract]:capability_unavailable` |
| `rel.privileged-operations` | `1` | `unavailable:fail_closed`; `policy_denied:fail_closed`; `slow_or_timed_out:reconciliation_blocked`; `external_outcome_ambiguous:reconciliation_blocked`; `saturated[recovery_capacity_state=outside_accepted_reservation]:shed_or_reject`; `saturated[recovery_capacity_state=inside_accepted_reservation]:queued_or_deferred` |

For `rel.control-plane-placement@1`, `placement_fallback_state` is a mandatory machine-evaluable selector with exactly two values: `verified_unexpired_lease_and_destination_admitted` and `fallback_ineligible`. The first requires a cryptographically/authoritatively verified, tenant- and generation-bound, unexpired placement lease plus current destination admission. `fallback_ineligible` is mandatory if either condition is absent, expired, unverifiable or contradicted. Only the first value permits bounded stable already-admitted traffic through `stale_tolerant`; the second always yields `fail_closed`. Process-local cache presence, wall-clock guess, circuit state or reachability cannot supply this selector.

The remaining state-qualified bindings use closed two-value selectors: `placement_cache_fallback_state={verified_unexpired_copy_and_destination_admitted,fallback_ineligible}`; `stale_read_state={product_authorized_within_freshness_contract,not_authorized_or_outside_contract}`; `configuration_fallback_state={verified_permitted_last_known_good,fallback_ineligible}`; `intent_state={not_committed,committed}`; `provider_path_state={product_authorized_durable_path,no_authorized_durable_path}`; `stale_result_state={product_authorized_within_disclosure_contract,not_authorized_or_outside_contract}`; and `recovery_capacity_state={outside_accepted_reservation,inside_accepted_reservation}`. Each selector is derived from the same scoped accepted authority record named by its profile, never from framework defaults, queue presence, reachability or process-local guesses. Every value's exact mode is materialized above and exercised by the profile's bound vector. `recovery_capacity_state=inside_accepted_reservation` is ineligible until `OPEN-REL-010` has an accepted Product/Phase 13 reservation disposition and evidence for the same role/workload scope; before that closure the selector SHALL resolve to `outside_accepted_reservation`.

For `rel.customer-telemetry-acceptance@1`, `acceptance_state` is a mandatory machine-evaluable selector with exactly two values: `not_durably_accepted` and `durably_accepted`. It is derived only from the canonical durable-acceptance record for the same scoped observation identity. Before that record exists, saturation rejects or sheds the unaccepted observation without acknowledgement. After it exists, saturation cannot reject, shed or reclassify the accepted observation; it yields `queued_or_deferred` for its bounded projection obligation while preserving the accepted identity, checkpoints, transition intent and consumed budget. No timeout, queue state, provider response or process-local flag may substitute for this selector.
...
|---|---|
| every `rel.*` profile and every version | `FV-RETRY-001`, `FV-RETRY-002`, `FV-RETRY-003`, `FV-COMP-001`, `FV-EVID-001`, `FV-AI-001`; `RB-REL-021`, `RB-REL-022`, `RB-REL-024`, `RB-REL-025`; `OPEN-REL-005`, `OPEN-REL-006`, `OPEN-REL-021`, `OPEN-REL-022`, `OPEN-REL-023`, `OPEN-REL-026`, `OPEN-REL-028` |

The selector is normative and mechanically expands to every catalog key; it is not an optional inheritance rule. A profile version cannot opt out of deadline, retry, compatibility or evidence-integrity coverage. Circuit evidence and the circuit implementation OPEN are derived separately from the exact circuit selector below so that a declared `no_applicable_case` inherits its mandatory negative evidence without inheriting an impossible half-open/probe implementation decision.
...
ALL-FAULT-VECTORS(profile_key) = distinct(
  ...
  union CIRCUIT-EVIDENCE(profile_key)
  union cross_profile_selector.fault_vectors
)

CIRCUIT-EVIDENCE(profile_key) =
  if exact_circuit_selector[profile_key].counted_failure_classes is a non-empty canonical set
  then circuit_policy_registry[selected_circuit_policy].evidence_vectors
  else exact_circuit_selector[profile_key].no_applicable_case.evidence_vectors

CIRCUIT-OPEN(profile_key) =
  if exact_circuit_selector[profile_key].counted_failure_classes is a non-empty canonical set
  then {OPEN-REL-007}
  else {}
...
ALL-OPEN-DECISIONS(profile_key) = distinct(
  profile_fault_fields[profile_key].profile_specific_open_decisions
  union cross_profile_selector.open_decisions
  union CIRCUIT-OPEN(profile_key)
)
```

Every referenced vector, blocker and OPEN SHALL exist. Each vector SHALL resolve to exactly one blocker row in `09-reliability-validation-and-fault-matrix.md`, and each selected OPEN SHALL resolve to exactly one accountable row in `12-phase-11-open-decisions-and-blockers.md`. `CIRCUIT-EVIDENCE` and `CIRCUIT-OPEN` SHALL take the same applicability branch. A `no_applicable_case` selector cannot import applicable-circuit probe/open vectors or `OPEN-REL-007`; it SHALL provide the dedicated negative-evidence branch instead. Static conformance SHALL reject any profile for which an element of `ALL-FAULT-VECTORS` is absent from evidence, its vector-derived blocker is absent from `ALL-RELEASE-BLOCKERS`, or its final `ALL-OPEN-DECISIONS` contradicts circuit applicability.
...
| `rel.customer-telemetry-acceptance@1` | `unavailable`, `saturated` | `unavailable→queued_or_deferred` within a bounded unacknowledged intake path or the durable accepted-observation backlog; `saturated[acceptance_state=not_durably_accepted]→shed_or_reject` without acknowledgement; `saturated[acceptance_state=durably_accepted]→queued_or_deferred` for the bounded projection obligation, never rejection or shedding | canonical durable acceptance record/journal, accepted scoped identity, projection checkpoints and current monotonic transition intent only |
...
| `rel.reporting-derived@1` | `unavailable`, `slow_or_timed_out`, `saturated` | `unavailable→queued_or_deferred`; `slow_or_timed_out→queued_or_deferred`; `saturated→shed_or_reject`; excluded `stale` follows `stale_result_state` profile binding and cannot be admitted by circuit fallback | authoritative source plus current rebuild/work generation only |
...
Each row below materializes profile-specific seed values. The complete `fault_vectors` and `release_blockers` fields SHALL be evaluated by `ALL-FAULT-VECTORS(profile_key)` and `ALL-RELEASE-BLOCKERS(profile_key)` above; therefore binding-derived and circuit-derived vectors necessarily add both evidence and their canonical blocker. The complete `open_decisions` field SHALL be evaluated by `ALL-OPEN-DECISIONS(profile_key)` above; `OPEN-REL-007` enters that join only when the exact circuit selector has a non-empty applicable failure-class set. These joins are explicit for every `rel.*@version` key, not optional inheritance. Compatibility values use only the classes in `10-compatibility-and-change-classification.md`.
...
| `rel.security-session-authority@1` | `FV-SEC-001`, `FV-COMP-001`, `FV-EVID-001`, `FV-AI-001` | `security_breaking`, `behavior_breaking`, `recovery_breaking` | `RB-REL-004`, `RB-REL-021`, `RB-REL-022`, `RB-REL-024` | `OPEN-REL-021`, `OPEN-REL-023`, `OPEN-REL-026`, `OPEN-REL-027`, `OPEN-REL-028`, `OPEN-REL-031` |
...
| `rel.privileged-operations@1` | `FV-PRIV-001`, `FV-REC-003`, `FV-COMP-001`, `FV-EVID-001`, `FV-AI-001` | `security_breaking`, `recovery_breaking`, `behavior_breaking`, `capacity_risk` | `RB-REL-020`, `RB-REL-021`, `RB-REL-022`, `RB-REL-024` | `OPEN-REL-010`, `OPEN-REL-014`, `OPEN-REL-021`, `OPEN-REL-024`, `OPEN-REL-027`, `OPEN-REL-023`, `OPEN-REL-026`, `OPEN-REL-028` |
