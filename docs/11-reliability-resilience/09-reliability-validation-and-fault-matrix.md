...
| `FV-TEL-002` | Parameterized customer-monitoring vector SHALL independently inject all six declared classes: `unavailable` at the acceptance boundary and after acceptance, `saturated` before and after acceptance, `duplicate`, `identity_conflict`, `out_of_order_or_gap`, and `recovery_continuity_blocked`; it also crashes after current-state advancement before ordinary signal publication and overlaps relocation. Aggregate pass requires every case. | Exact modes are mandatory: `unavailable→queued_or_deferred` without pre-acceptance acknowledgement and with accepted responsibility preserved; `saturated→shed_or_reject` only before durable acceptance while accepted observations remain queued; `duplicate→fail_fast` through scoped dedup without reapplying history/projection; `identity_conflict→reconciliation_blocked`; `out_of_order_or_gap→reconciliation_blocked` for affected current-state/signal projection while historical acceptance remains durable; `recovery_continuity_blocked→reconciliation_blocked`. Every case preserves canonical scoped identity, monotonic projection, durable transition intent and one source generation. | Optional shedding or false acknowledgement after acceptance; any injected class producing another mode; provider-local ID collapsing tenants/sources; duplicate historical observation; stale replay regressing current state; committed advance losing its signal; unbounded projection backlog; dual source/target acceptance. | Per-class result records prove the exact mode and boundary; acceptance/projection watermarks, scoped dedup identity, monotonic tokens, durable transition intent and relocation fence prove convergence per tenant/source/generation while optional telemetry and transactional core remain isolated. | `RB-REL-026` |
