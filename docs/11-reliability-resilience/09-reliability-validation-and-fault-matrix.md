...
| `FV-TEL-002` | Customer-monitoring ingestion loses the durable acceptance boundary, a projection after acceptance, identity/checkpoint continuity, or crashes after current-state advancement before ordinary signal publication; duplicate/out-of-order replay and relocation overlap are included. | Before acceptance, defer or reject boundedly without acknowledgement. After acceptance, preserve the canonical scoped observation and transition obligations, replay idempotently, keep historical acceptance valid, advance current state only by monotonic ordering token, and reconcile checkpoints/source generations before resumption. | Optional shedding or false acknowledgement after acceptance; provider-local ID collapsing tenants/sources; duplicate historical observation; stale replay regressing current state; committed advance losing its signal; unbounded projection backlog; dual source/target acceptance. | Acceptance and projection watermarks, scoped dedup identity, monotonic tokens, durable transition intent and relocation fence prove convergence per tenant/source/generation while optional telemetry and transactional core remain isolated. | `RB-REL-026` |
