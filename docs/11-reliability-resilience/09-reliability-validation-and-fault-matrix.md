...
| `FV-CIRCUIT-002` | Circuit opens while recovery, security-control or other protected critical work requires the same dependency. | Apply the selected profile's explicit open/fallback policy; reserve only accepted bounded critical capacity and never bypass current authority, ambiguity or recovery gates. | Circuit-open as effect absence; generic fallback granting authority; critical label bypassing tenant/governance; backlog starvation; state reset on restart. | Critical and ordinary work remain attributable/isolated; fallback authority and resumption evidence match the exact profile/generation. | `RB-REL-025` |
