...
| `FV-TEL-001` | Optional telemetry path is saturated or unavailable. | Keep telemetry non-authoritative and deterministically `shed_or_reject` under the optional telemetry profile; record explicit bounded loss. | Fabricated completeness; optional telemetry blocking business truth; unbounded buffer/cardinality; secret/tenant leak. | Loss remains explicit and attributable; core isolation and data classification are proven. | `RB-REL-017` |
| `FV-AUDIT-001` | Mandatory audit responsibility cannot be durably established at a protected-effect boundary. | Enter `unavailable:fail_closed` for the affected protected effect; optional telemetry policy cannot override this profile. | Protected effect without mandatory audit responsibility; downgrade to optional telemetry; fabricated or later-reconstructed audit success; cross-tenant audit scope. | Effect and audit identity/scope remain joined; unrelated non-audit-gated work remains isolated; admission resumes only after durable audit continuity. | `RB-REL-017` |
