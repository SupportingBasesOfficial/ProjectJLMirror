...
`broker_boundary.py` is protected by two layers. The repository validator scans the boundary surface, and the independent structural guard `validate_structural_boundary_guards.py` requires the native exception to bind to **exactly one top-level lexical declaration** named `KafkaCandidateAdapter`. A second top-level declaration with that name, a nested declaration with that name, or any ambiguity in the exemption fails closed. Only the adapter's direct method implementation bodies receive the candidate-native exception; every nested lexical declaration inside those methods — nested class, `def`, `async def`, or `lambda` — is independently scanned and cannot inherit native Kafka/transaction authority. Adapter decorators, bases and class keywords also remain governed. `AlternateStubTransport`, broker paths, helpers, module-level code and all other classes therefore remain inside the native Kafka/transaction prohibition.
...
The ordered call-set check remains a useful secondary signal, but the verify-before-ack claim no longer depends on lexical source order. The independent structural guard requires `InboxAcknowledgePath.acknowledge_after_durable_responsibility` to be an **undecorated** linear two-statement authority path: unconditional `_verifier.assert_durable(receipt)` followed by `_broker.acknowledge(...)`. The containing `InboxAcknowledgePath` class must also be undecorated. This prevents a decorator or wrapper from replacing the analyzed class/method and acknowledging on a feasible path that bypasses the verified body. Additional branches, `try` blocks, loops, early-return paths, broker-first order, or conditional verification are rejected. This establishes structural dominance for the currently governed source shape instead of assuming that an earlier line dominates a later call.

Negative controls explicitly falsify the historical escapes: duplicate/nested allowlisted adapter declarations are rejected; nested `def`, `async def` and `lambda` helpers cannot hide transaction APIs inside the adapter exception; a conditional verifier such as `if receipt.receipt_id != "escape": assert_durable(...)` followed by broker acknowledgement is rejected even though its textual call order appears correct; broker-first order is rejected; and decorated ack methods/classes are rejected as replaceable authority surfaces. Separately, a matching-semantics forged-receipt runtime control proves that a receipt absent from durable state cannot remove the queued delivery.
...
The package is source-evidence-ready only after exact-HEAD CI and fresh adversarial review prove current-namespace discovery, distinct same-name nested declaration accounting, multi-target-assignment-alias-aware direct/indirect broker-path inheritance discovery, **single-top-level-lexical-declaration** native allowlisting limited to `KafkaCandidateAdapter` with every nested lexical declaration still scanned, executable transitive assurance closure including parent-package initializers, owner-independent polyglot Kafka transaction-API rejection including transaction initialization plus snake/camel/Pascal and language-specific generic syntax normalization, exact logical call-graph closure plus **undecorated structural unconditional dominance** of durable verification over acknowledgement, canonical versioned record reconstruction, durable effect/ack boundary, fail-closed governed consumer-registry path validation, executable registration binding, semantic payload/effect equivalence and resolved source-run provenance. Merge of this PR still credits **0/7** D4-A evidence.