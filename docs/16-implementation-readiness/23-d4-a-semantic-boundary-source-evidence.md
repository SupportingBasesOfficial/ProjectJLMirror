...
The containing `InboxAcknowledgePath` is constrained to the source shape actually reviewed: it must be undecorated, inherit directly and only from `BrokerFacingPath`, use no metaclass/class keywords, contain no class-body rebinding assignments, and define no replacement-capable special methods such as `__getattribute__`, `__getattr__`, `__setattr__`, descriptor hooks or subclass/class lookup hooks. These restrictions prevent decorators, metaclasses, entrypoint rebinding and special-method interception from substituting the analyzed acknowledgement method at runtime. Additional branches, `try` blocks, loops, early-return paths, broker-first order, async replacement, or conditional verification are rejected. This establishes structural dominance for the currently governed source shape rather than assuming an earlier line dominates a later call.

Negative controls explicitly falsify historical and newly discovered escapes: duplicate/nested allowlisted adapter declarations are rejected; nested `def`, `async def` and `lambda` helpers cannot hide transaction APIs; class-body statements, default arguments, decorator expressions and annotation expressions cannot acquire transaction authority during class/method construction; conditional and broker-first acknowledgement fail; decorated ack methods/classes fail; metaclass replacement, class-body entrypoint rebinding and `__getattribute__` interception fail; and destructuring aliases of broker-path bases are mechanically recovered. Separately, a matching-semantics forged-receipt runtime control proves that a receipt absent from durable state cannot remove the queued delivery.
...
The package is source-evidence-ready only after exact-HEAD CI and fresh adversarial review prove current-namespace discovery, distinct same-name nested declaration accounting, **simple/chained/annotated/destructuring-alias-aware** direct/indirect broker-path inheritance discovery, a **single-top-level lexical** native adapter declaration whose exception is limited to direct method bodies while all class/method construction-time surfaces and nested lexical declarations remain scanned, executable transitive assurance closure including parent-package initializers, owner-independent polyglot Kafka transaction-API rejection including transaction initialization plus snake/camel/Pascal and language-specific generic syntax normalization, exact logical call-graph closure plus **non-replaceable structural unconditional dominance** of durable verification over acknowledgement, canonical versioned record reconstruction, durable effect/ack boundary, fail-closed governed consumer-registry path validation, executable registration binding, semantic payload/effect equivalence and resolved source-run provenance. Merge of this PR still credits **0/7** D4-A evidence.