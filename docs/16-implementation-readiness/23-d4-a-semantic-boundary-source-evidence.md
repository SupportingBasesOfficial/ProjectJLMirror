...
Native Kafka SDK usage and Kafka transaction APIs such as begin/commit/abort/send-offsets-to-transaction are rejected across that closure and across every governed source language in `CODE_SUFFIXES`. Python uses AST attribute detection. Non-Python sources inspect member-call identifiers and normalize case plus underscore differences before comparison, so snake_case, camelCase and PascalCase spellings such as `commit_transaction`, `commitTransaction`, `CommitTransaction`, `sendOffsetsToTransaction` and `SendOffsetsToTransaction` map to the same prohibited transaction authority. Detection remains owner-name independent, so aliasing an injected client as `client`, `transport`, `session`, or another arbitrary variable cannot hide those APIs.
