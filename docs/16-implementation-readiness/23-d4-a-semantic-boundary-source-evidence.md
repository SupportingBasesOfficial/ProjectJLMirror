...
Native Kafka SDK usage and Kafka transaction APIs such as begin/commit/abort/send-offsets-to-transaction are rejected across that closure and across every governed source language in `CODE_SUFFIXES`. Python uses AST attribute detection; non-Python sources use owner-independent transaction-call detection in addition to native SDK markers. Aliasing an injected client as `client`, `transport`, `session`, or another arbitrary variable therefore cannot hide those transaction APIs merely by changing language or variable name.
