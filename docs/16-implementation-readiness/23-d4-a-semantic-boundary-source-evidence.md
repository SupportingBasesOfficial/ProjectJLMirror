...
`boundary-inventory.json` independently pins the four expected broker-facing paths, D4 code roots, consumer-discovery root and registration entrypoint. `validate_repository_boundary.py` independently pins those values again, mechanically discovers every direct **and indirect** `BrokerFacingPath` descendant across governed Python sources, follows imported aliases of both the abstract boundary base and already-known concrete descendants, requires exact inventory equality/multiplicity, scans governed implementation code for direct Kafka SDK/native bypass, and pins the exact dependency call graph of every logical path. A subclass of an existing broker-facing path therefore remains governed even if that base is imported under a caller-chosen alias such as `OutboxDispatchPath as Base`.

The no-Kafka-business-authority scan walks the local assurance dependency closure **transitively** from the durable effect verifier and consumer-registration guard. Complete local module paths are resolved, including nested subpackages and relative imports, so a helper such as `helpers.guard` or a deeper imported helper is scanned automatically rather than being reduced to an unresolvable leaf filename. Native Kafka SDK usage and Kafka transaction APIs such as begin/commit/abort/send-offsets-to-transaction are rejected across that closure. Transaction-API detection is owner-name independent: aliasing an injected Kafka client as `client`, `transport`, `session`, or another arbitrary variable cannot hide the transaction authority.
