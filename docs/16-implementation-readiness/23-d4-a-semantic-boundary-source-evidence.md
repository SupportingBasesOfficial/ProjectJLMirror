...
`broker_boundary.py` itself is scanned class-by-class. Only the independently pinned `KafkaCandidateAdapter` body is exempt from native Kafka/transaction rejection; `AlternateStubTransport`, broker paths, helpers, module-level code and all other classes remain inside the prohibition. This prevents the evidence adapter exception from becoming a module-wide escape hatch.
...
The proof now pins both the exact dependency set and the ordered call sequence. For `InboxAcknowledgePath`, `_verifier.assert_durable` must precede `_broker.acknowledge`; the same unordered set in broker-first order fails the source proof. A separate matching-semantics forged-receipt negative control proves runtime fail-closed behavior: even when consumer, scope, message ID, contract, version and semantic digest all match the queued delivery, a receipt absent from durable state is rejected before broker acknowledgement and the message remains queued.
...
The package is source-evidence-ready only after exact-HEAD CI and fresh adversarial review prove current-namespace discovery, distinct same-name nested declaration accounting, multi-target-assignment-alias-aware direct/indirect broker-path inheritance discovery, broker-boundary class-scoped native allowlisting limited to `KafkaCandidateAdapter`, executable transitive assurance closure including parent-package initializers, owner-independent polyglot Kafka transaction-API rejection including transaction initialization plus snake/camel/Pascal and language-specific generic syntax normalization, **ordered** exact logical call-graph closure with durable verification before acknowledgement, canonical versioned record reconstruction, durable effect/ack boundary, fail-closed governed consumer-registry path validation, executable registration binding, semantic payload/effect equivalence and resolved source-run provenance. Merge of this PR still credits **0/7** D4-A evidence.
