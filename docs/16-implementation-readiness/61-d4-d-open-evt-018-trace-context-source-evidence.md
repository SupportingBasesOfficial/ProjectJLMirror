...
- authenticated tenant-scope proof for repeated internal async hops: byte equality or a short tenant-derived prefix is never accepted as proof that a trace is already scoped; the HMAC attests the exact tenant plus the tenant-scoped trace identity, while the W3C parent/span ID remains free to change on a legitimate child span without breaking same-tenant trace continuity;
...
- proof-bound same-tenant continuity: a valid attestation preserves the tenant-scoped trace ID across repeated hops while allowing each trusted hop to emit a new valid parent/span ID; the same attestation crossing into another tenant boundary causes a new tenant re-scope before export;
