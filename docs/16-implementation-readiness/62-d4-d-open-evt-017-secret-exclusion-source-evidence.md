...
The source harness treats ordinary messages as reference-only consumers of external secret/KMS authority. Every ordinary payload field carries an explicit material classification. Only `public`, `internal`, and `business_data` classifications are admitted; `secret`, `credential`, `key_material`, unknown classifications, and typed `SecretMaterial` values are rejected. A successfully resolved secret is returned as `SecretMaterial` and its handle is registered as resolved secret authority, so later type stripping cannot make that handle an ordinary safe string merely because the destination message has no secret reference or a different reference.
...
Text values are evaluated against all known resolved secret handles plus the message-selected handle when present. JSON-looking text beginning with `{` or `[` is deserialized and recursively evaluated under the same mapping/positional rules. Dedicated parser-only fixtures use an unrelated opaque handle and no attached secret reference, so removing JSON decoding itself makes those probes fail; the structural JSON revalidation is therefore independently required rather than shadowed by the raw-handle substring guard. Unsupported opaque Python objects are rejected rather than assumed safe.

After successful secret resolution, the harness records the exact resolved handle in a deny registry representing resolved-secret authority knowledge. Ordinary payload admission consults that registry independently of `message.secret_ref`. Consequently, a previously resolved handle is rejected when placed directly in `business_data`, when `secret_ref=None`, when another secret reference is attached, when embedded in text, or when used as a top-level or nested object key.
...
The corrected harness executes an exact validator-pinned set of **94** positive and negative probes covering:
...
- direct, positional, and nested textual JSON serialization rejection, plus rejection of opaque text fragments containing known resolved handles;
...
- top-level and nested object-key rejection when a key contains a selected or previously resolved secret handle;
- rejection of a previously resolved handle when the destination message has no attached secret reference or a different attached reference;
...
- non-empty string-only message/tenant identifiers, rejection of identifiers that equal or embed a selected or previously resolved secret handle, and reconstructed sanitize/erasure coverage for handle-bearing identifiers;
