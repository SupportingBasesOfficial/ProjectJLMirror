...
A shared message-boundary validator is applied at creation, sanitization, and erasure. Message and tenant identifiers must be literal non-empty strings and must not equal or embed a selected or durably known secret-authority handle before they can be copied into ordinary or secondary records. Secret-reference handles are also required to be literal non-empty strings **before** namespace operations; malformed reconstructed/deserialized handles therefore fail with `SecretBoundaryDenied` rather than leaking `AttributeError`/type-dependent implementation behavior. This blocks both typed secret objects and type-stripped secret-handle strings in reconstructed/deserialized identifier slots. The same shared boundary prevents reconstructed/deserialized `OrdinaryMessage` instances from bypassing payload, secret-reference, verification-reference, and verification-generation invariants that apply at creation time.
...
The corrected harness executes an exact validator-pinned set of **104** positive and negative probes covering:
...
- direct, positional, and nested textual JSON serialization rejection, URL/form serialization rejection after percent-decoding, plus rejection of opaque text fragments containing known secret-authority handles;
...
- independent secret-reference literal-string handle, strict integer generation, scope, reserved verification namespace, reserved audit namespace, and bearer guards, including creation and resolution probes for malformed non-string handles;
