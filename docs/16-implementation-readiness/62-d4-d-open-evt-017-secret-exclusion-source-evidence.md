...
The corrected harness executes an exact validator-pinned set of **112** positive and negative probes covering:
...
- direct, positional, nested, escaped-scalar, and duplicate-member textual JSON rejection; URL/form serialization rejection after percent-decoding; single- and double-percent-escaped serialization rejection under bounded iterative normalization; benign double-escape acceptance after stabilization; explicit fail-closed rejection beyond the decode-round bound; plus rejection of opaque text fragments containing known secret-authority handles;
- parser-only mapping, positional, nested, scalar, and duplicate-member JSON fixtures that require the JSON decoder/revalidation path itself and preserve every object member before duplicate-name collapse;
