...
The durable authority-handle catalog is independent of process-local resolution history. Dedicated fresh-worker evidence attempts to place the configured `kms://orders-signing/current` handle into `business_data` with `secret_ref=None` before relying on any prior resolution history and requires fail-closed admission. The same authority knowledge is consulted by payload values, object keys, identifiers, and verification references. A dedicated `verification-profile://collision-profile` fixture has no attached `secret_ref`, so the verification-reference authority collision can only be rejected by the durable authority-handle guard.
...
The corrected harness executes an exact validator-pinned set of **96** positive and negative probes covering:
