...
Text values are evaluated against the durable authority-handle catalog plus the message-selected handle when present. Any valid JSON text is deserialized and recursively evaluated under the same rules, including objects, arrays, numbers/booleans/null, and JSON scalar strings. JSON objects are also parsed through an `object_pairs_hook` representation that preserves every member in source order before duplicate names can collapse. Each pair-preserved object is first checked as an object for the same structural `handle` + strictly positive integer `generation` secret/reference shape used by ordinary mappings, and only then are its individual decoded key/value pairs recursively inspected. This prevents both an earlier escaped bearer-handle member such as `{"x":"kms:\/\/orders-signing\/current","x":"benign"}` and an earlier structurally serialized secret object such as `{"x":{"handle":"opaque-parser-only","generation":11},"x":"benign"}` from disappearing behind the final duplicate value. Escaped JSON scalar representations such as `"kms:\/\/orders-signing\/current"`, whose raw text does not contain the configured handle but whose decoded scalar does, are rejected as well. URL/form-looking text is normalized with form decoding before the same secret-handle/serialized-material rules are applied. Standalone percent-escaped text is normalized iteratively for at most four decode rounds, with a decoded-size bound; every decoded layer is reinspected for known handles, JSON secret/reference material, or form serialization. If input remains meaningfully percent-decodable after the depth bound, the boundary fails closed instead of accepting over-encoded text. This prevents both `quote(json.dumps(asdict(resolve_secret(...))), safe="")` and `quote(quote(json.dumps(asdict(resolve_secret(...))), safe=""), safe="")` from bypassing the boundary simply because intermediate layers contain no raw handle or form pair. The causality of this boundary is pinned in both directions: a benign doubly percent-escaped payload must be accepted after iterative stabilization, proving a second decode round is functionally required rather than masked by the post-bound fallback, while a secret payload escaped for `URL_DECODE_MAX_ROUNDS + 1` layers must fail closed specifically because meaningful decoding remains after the configured bound. Dedicated parser-only JSON fixtures use unrelated opaque handles and no attached secret reference; the JSON-scalar fixture uses escaped slashes around a configured handle with `secret_ref=None`; one duplicate-member fixture places the escaped configured handle in the earlier of two identically named members; and a second duplicate-member fixture places a structurally serialized `handle`/`generation` object in the overwritten member. Removing either pair-preserving traversal or the pair-object structural check makes the corresponding probes fail. A dedicated URL/form probe uses `urlencode(asdict(resolve_secret(...)))` with `secret_ref=None`; separate single-, double-, benign-double-, and over-bound percent-escaped probes require the bounded iterative URL-normalization path rather than raw substring matching. Unsupported opaque Python objects are rejected rather than assumed safe.
...
The corrected harness executes an exact validator-pinned set of **113** positive and negative probes covering:
...
- direct, positional, nested, escaped-scalar, duplicate-member bearer-handle, and duplicate-member structural-secret textual JSON rejection; URL/form serialization rejection after percent-decoding; single- and double-percent-escaped serialization rejection under bounded iterative normalization; benign double-escape acceptance after stabilization; explicit fail-closed rejection beyond the decode-round bound; plus rejection of opaque text fragments containing known secret-authority handles;
- parser-only mapping, positional, nested, scalar, and duplicate-member JSON fixtures that require the JSON decoder/revalidation path itself, preserve every object member before duplicate-name collapse, and apply structural mapping checks to pair-preserved objects before iterating members;
