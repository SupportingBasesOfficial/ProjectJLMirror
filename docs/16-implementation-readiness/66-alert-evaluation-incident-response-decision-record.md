...
This document closes the "Alert Policy/Evaluation" open question from `OPEN-QUESTIONS-AND-DEFERRED.md` and introduces `ApplicationErrorEvent` as a new authorized inbound event type and `IncidentResponsePolicy` as the per-tenant automatic response configuration.

## Context and problem

`OPEN-QUESTIONS-AND-DEFERRED.md` lists: "exact Alert Policy/Evaluation DSL/model remains unaccepted; exact grouping/correlation/dedupe semantics remain unaccepted; automatic Alert create/resolve remains blocked." This record closes the policy model for G11+ implementation, while keeping G10-and-below semantics unchanged.
...
### Alert evaluation model — three inbound event types

Alert evaluation is triggered by exactly these authorized event types:
...
#### Type 3 — ApplicationErrorEvent (NEW, authorized by this record)
...
4. `notify_channels` entries are evaluated in order; each matching severity entry dispatches a notification via the authorized NotificationPort (IR-D-067).
...
### Automatic Alert create/resolve authority

This record authorizes automatic Alert create and resolve under these conditions:
...
Before the push API is production-eligible:
...
Before auto-ticket is production-eligible:
