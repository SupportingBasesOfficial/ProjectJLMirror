...
### Phase A — PostgreSQL role-per-worker (proposed monolith profile)

If this record is accepted, the monolith profile would require:
...
If this record is accepted and its closure evidence passes, Phase A would satisfy the bounded OPEN-PRT-008 monolith residual without coupling to an external workload-identity product. This proposed record grants no implementation authority by itself.
