...
This document is **not** a new ADR. It selects PostgreSQL as the canonical BFF session fence store and fixes the cache/invalidation semantics required by IR-D-001 and OPEN-REL-015.

## Context and problem

IR-D-001 fixes that "logout/revocation/session retirement invalidates server-side session authority even if a browser cookie remains present" and that "the actual security-cache propagation/fencing must conform to `OPEN-REL-031-session-store-decision-record.md` and the canonical `OPEN-REL-015` cache invalidation/epoch ownership." This record is the OPEN-REL-031 resolution: it selects the backing store and fixes the in-process caching/invalidation contract.
...
### Canonical fence store — PostgreSQL

The BFF session fence store is PostgreSQL (same TimescaleDB instance used by the monitoring subsystem, in a dedicated `auth` schema with restricted roles).
...
- Cache TTL is 30 seconds; this is the maximum staleness for a session retirement to be observable from the cache perspective. The acceptable propagation bound is therefore 30 seconds in the current monolith (all workers share one process cache).
