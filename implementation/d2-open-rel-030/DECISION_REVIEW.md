...
10. all current history hardening modules `004–009` execute under an anti-orphan workflow guard;
11. physical PITR admission derives from authenticated surviving authority external to restored state;
12. actual post-R effect evidence is authenticated, grant-bound and locally applied only after verified material fetch;
13. one canonical recovery-boundary winner exists across equivalent grant IDs;
14. the surviving active authority tuple is locked before winner derivation and revalidated by claim/verify/fetch;
15. the winner is bound to authenticated principal plus non-PGDATA effective restored-instance proof;
16. clone rejection requires same-path positive control;
17. recovery material fetch uses a consistent locked authority→grant→claim→effect/signing snapshot;
18. the hardened positive recovery path is exercised after hardening installation;
19. established recovery/clone response deadlines are caller-local, asynchronous and fail closed without synchronous timeout cleanup;
20. real established TCP blackholes are directly falsified;
21. deterministic versioned self-delimiting serialization and total/injective typed canonicalization precede hashes/MACs/signatures;
22. Timescale Tier 2 is selected only under the mediated shared-history profile proven by C2 evidence;
23. `ts_automation_owner` remains cross-tenant privileged infrastructure, excluded from tenant/application auth;
24. fresh-cluster role reconstruction and post-restore/job attack matrices are mandatory;
25. source relocation authority is locked before F and target completeness is canonical-set completeness, never max-only;
26. target checkpoint measurement/signing authority originates target-side; Tier 1 verifies but cannot mint;
27. verifier credentials remain restricted capability state, absent from function source;
28. effective final cross-authority verifier transport has caller-local response deadlines and no synchronous timeout cancel/disconnect;
29. real response blackholes are exercised in both relocation directions;
30. Tier 1 successor placement and exact activation grant commit atomically;
31. target `sealed → activated` requires that exact committed Tier 1 grant;
32. pre-activation future rows are rejected; activated existing history is immutable and only new append `>F` is eligible;
33. `OPEN-REL-020` remains owner of production capacity/SLO/retention/cardinality/cost numerics;
34. evaluation database versions, image digests, evidence HMACs, LOGIN roles, external-to-PGDATA mount, `dblink`, one-shot session retirement and laboratory deadlines remain reproducibility dependencies, not frozen production selections.
...
Exact #186 verifies the anchor SHA, reports six history hardening modules, preserves the complete #38–#49 recovery/clone/Timescale/relocation/history package and adds class #50 proof:
...
For a stream with no existing `finalized_through`, a worker could call `try_finalize(stream_id,NULL)`. The former finalizer derived `v_required_through=NULL`; comparisons such as `v_required_through > finality_floor` and `current_covered < v_required_through` evaluated to SQL `NULL`, not true. The rejection branch could therefore be skipped, allowing `state=complete` with `finalized_through=NULL`.

### Closure

The effective final `try_finalize(...)` rejects `p_finalize_through IS NULL` immediately using SQLSTATE `22004` before state/authority reads and before any three-valued cutoff comparison. The regression vector establishes valid short coverage on a never-finalized stream, submits the NULL cutoff, requires the explicit rejection, and proves the existing `provisional` state and coverage remain unchanged with `finalized_through IS NULL`. Malformed input cannot mint completeness.

This closes Codex P1 thread `PRRT_kwDOT7x07M6ddhwC` at the mechanism/evidence layer. Governance promotion and fresh exact-final-head review are separate gates.

## Class #49 — retained finalized watermark revalidation

A durable historical T2 watermark cannot be restored to current `complete` after provider-dataset invalidation using only shorter T1 coverage. The finalizer requires current generation + dataset revision + required snapshot contiguous coverage through `max(requested_finalize_through, existing_finalized_through)`. Exact #186 preserves both #49 regression markers.

## Material finding classes closed by D2

The evidence program has repaired **50 material classes**, with panoramic review after each repair:
...
Inline review threads        0 unresolved after #50 evidence closure
