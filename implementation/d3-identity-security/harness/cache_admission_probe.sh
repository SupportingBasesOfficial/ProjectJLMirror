...
CACHE_CLI="${CACHE_CLI:?CACHE_CLI is required}"
...
cache() {
  docker exec "$CACHE_CONTAINER" "$CACHE_CLI" --raw "$@"
}
...
cache_eval() {
  ...
  docker exec "$CACHE_CONTAINER" "$CACHE_CLI" --raw EVAL '
...
# Healthy hit is one EVAL request. Expected authority inputs are caller-local;
...
# The expected epoch is outside the cache. A stale cache dataset cannot self-certify currentness.
EXPECTED_EPOCH=2
...
# Logical stale-snapshot repopulation deliberately avoids a product-specific RDB format.
# The invariant under test is that resurrected positive bytes still cannot self-certify a
# newer externally-held authority generation/epoch.
cache HSET session:restored \
  session_gen 2 principal_gen 1 membership_gen 1 permission_gen 1 \
  tenant_gen 1 access_gen 1 admission_epoch 2 >/dev/null
assert_eq DENY "$(cache_eval session:restored 3 1 1 1 1 1 3)" "${CACHE_LABEL}_restored_positive_cannot_self_authorize"

# If the local cache/fence authority is unavailable, orchestration refuses a new durable
# source mutation; PostgreSQL remains at generation/epoch 2.
docker stop "$CACHE_CONTAINER" >/dev/null
cache_available=false
if docker exec "$CACHE_CONTAINER" "$CACHE_CLI" PING >/dev/null 2>&1; then
