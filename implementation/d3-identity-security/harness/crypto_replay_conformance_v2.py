...
def scalar(container: str, sql: str, *, check: bool = True, timeout: int = 5) -> str:
    ...
    if len(lines) != 1:
        raise RuntimeError(f"expected exactly one SQL scalar, got {len(lines)} rows")
    return lines[0]


def bao(method: str, path: str, payload: dict | None = None, *, ok=(200, 204)) -> dict:
    raw_body = None if payload is None else json.dumps(payload, separators=(",", ":")).encode()
    ...
    try:
        ...
        raise RuntimeError(f"OpenBao {method} {path}: HTTP {exc.code}: {body[:500]}") from exc
...
class KeyAuthorityPort:
    """Evidence-only provider-neutral boundary; backend-native paths stay adapter-local."""
    ...
    def state(self) -> tuple[int, int, bool]:
        ...
        return int(current), int(floor), erased == "t"
    ...
    def generate_hmac(self, *, generation: int, message: bytes) -> str:
        ...
        if generation != current:
            raise RuntimeError("only current key generation may create new evidence")
        result = bao(
    ...
    def rotate(self) -> int:
        ...
        changed = scalar(
            PG_CONTROL,
            "UPDATE security_control.key_state SET current_generation=" + str(nxt)
        ...
        require(changed == str(nxt), "trusted key-generation CAS failed")
        return nxt
    ...
    def mark_erased(self) -> None:
        ...
        require(changed == "t", "trusted erasure tombstone was not created")
...
class ReplayAuthority:
    """Replay witness is security continuity truth; local replay table is only a serving mirror."""
...
def prove_historical_continuity() -> None:
    ...
    print("d3_e_historical_verifier_relocation_recovery_continuity=PASS source_generation_preserved=true target_verify_only=true current_generation_not_downgraded=true missing_manifest_fails_closed=true")
...
def prove_erasure_nonresurrection() -> None:
    ...
    for fn in (
        lambda: key.verify_hmac(generation=1, message=message, mac=old_mac),
        lambda: key.generate_hmac(generation=1, message=message),
    ):
        try:
            fn()
        except RuntimeError:
            ...
            raise AssertionError("restored erased key became canonical authority")
    print("d3_e_retired_erased_key_nonresurrection=PASS native_restore_negative_control_usable=true trusted_erasure_tombstone_external=true restored_key_not_current_authority=true restored_key_cannot_generate_or_verify=true")
...
def prove_replay_single_winner() -> None:
    ...
    def attempt(_: int) -> bool:
        ...
        return replay.consume(
            replay_scope="issuer-A|client-A", principal="machine-A",
            jti="assertion-jti-atomic", expected_generation=generation,
        )
    ...
    require(
        scalar(PG_CONTROL, "SELECT count(*) FROM security_control.replay_witness WHERE replay_scope='issuer-A|client-A' AND client_principal='machine-A' AND jti='assertion-jti-atomic';") == "1",
        "durable replay identity was not single-winner",
    )
    require(
        replay.consume(replay_scope="issuer-B|client-B", principal="machine-A", jti="assertion-jti-atomic", expected_generation=generation),
        "same raw jti incorrectly collided across trusted issuer/client scope",
    )
...
def prove_replay_partition() -> None:
    ...
    require(
        replay.consume(replay_scope="issuer-A|client-A", principal="machine-A", jti="post-partition-jti", expected_generation=generation),
        "replay authority did not recover",
    )
...
def prove_replay_restore_loss() -> None:
    ...
    generation = replay.current_generation()
    require(
        replay.consume(replay_scope="issuer-R|client-R", principal="machine-R", jti="assertion-before-restore", expected_generation=generation),
    ...
    require(
        not replay.consume(replay_scope="issuer-R|client-R", principal="machine-R", jti="assertion-before-restore", expected_generation=nxt),
        "consumed assertion became unused after restore",
    )
    require(
        replay.consume(replay_scope="issuer-R|client-R", principal="machine-R", jti="assertion-new-after-restore", expected_generation=nxt),
        "fresh assertion rejected after recovery",
    )
