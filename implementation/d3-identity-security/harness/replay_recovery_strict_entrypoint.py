...
def _require_exact_capability(expected: dict, observed: dict, outcome: str) -> None:
    if observed.get("outcome") != outcome:
        raise RuntimeError("provider outcome diverged from captured recovery continuity")
    fields = ("attempt_generation", "attempt_token", "revision")
    if outcome == "CONFIRMED":
        fields += ("effect_id", "result_ref")
    for field in fields:
        if observed.get(field) != expected.get(field):
            raise RuntimeError(f"provider continuity mismatch for {field}")
...
def _resolve_provider_capability_before_capture(op: str) -> dict:
    ...
    if state == "attempting":
        ...
        expected = "CONFIRMED" if status.get("effect") else "ABSENT"
        _require_exact_capability(capability, outcome, expected)
        if not core.reconcile_provider(op, generation, outcome):
...
def _rehydrate_missing_row(op: str, epoch: int, effect: dict | None, fence: dict | None) -> None:
    if effect and fence:
        raise RuntimeError("provider continuity cannot contain effect and absence fence together")
    ...
    expected = "CONFIRMED" if effect else "ABSENT"
    _require_exact_capability(capability, observed, expected)
    if effect:
        core.psql(
        ...
        core.psql(
            ...
            f"VALUES({core.lit(op)},{epoch},'prepared',{generation},{core.lit(fence['revision'])});"
        )
...
def recover_from_witness_strict(witness: core.RecoveryWitnessPort) -> None:
    ...
    for op, witnessed_status in payload["provider_outcomes"].items():
        ...
        if effect and fence:
            raise RuntimeError("captured provider continuity is contradictory")
        capability = effect or fence
        row = _redrive_row(op)
        if row is None:
            _rehydrate_missing_row(op, epoch, effect, fence)
            continue
        ...
        if state == "attempting":
            if not capability:
                raise RuntimeError("restored attempting row lacks captured provider continuity")
            if not restored_token or restored_token != capability["attempt_token"]:
                raise RuntimeError("restored attempting capability token mismatch")
            expected = "CONFIRMED" if effect else "ABSENT"
            observed = core.provider_probe(op, generation, restored_token)
            _require_exact_capability(capability, observed, expected)
            ...
            expected = "CONFIRMED" if effect else "ABSENT"
            observed = core.provider_probe(op, generation, capability["attempt_token"])
            _require_exact_capability(capability, observed, expected)
            if not core.reconcile_provider(op, generation, observed):
            ...
            observed = core.provider_probe(op, generation, effect["attempt_token"])
            ...
            if fence:
                observed = core.provider_probe(op, generation, fence["attempt_token"])
...
def prove_provider_restart_durability(
    ...
    ambiguous = core.provider_send(
        effect_op, 1, "restart-token", "restart-effect", "restart-result", drop=True
    )
...
def prove_whole_restore_strict(
    port: core.ReplayAuthorityPort, witness: core.RecoveryWitnessPort,
) -> None:
    ...
    assert surviving["epoch"] == 2 and surviving["admission_open"] is False
    assert port.consume(
        "restore-client", "restore-jti", "restore-fp",
        "restore-session-effect", "restore-session-result", 1,
    ) == "BLOCKED"
    ...
    print(
        ...
        "consumed_identity_rehydrated=true exact_provider_capability_bound=true "
...
def main() -> None:
    ...
    try:
        ...
        print(
            ...
            "exact_provider_capability_binding=true c3_numerics_not_selected=true topology_not_selected=true"
        )
        print(
            "d3_e_replay_recovery_entrypoint=PASS "
            "stale_db_cannot_self_certify=true external_recovery_epoch_wins=true "
            "attempting_rows_cannot_reopen=true capability_token_generation_revision_bound=true"
        )
    finally:
        stop_provider(provider_proc)
        core.WITNESS_PATH.unlink(missing_ok=True)
