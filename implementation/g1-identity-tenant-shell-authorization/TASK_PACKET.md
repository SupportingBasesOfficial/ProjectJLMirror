...
IMPLEMENTATION DIFF GATE: .github/workflows/g1-identity-tenant-shell-implementation-scope.yml uses pull_request_target so the workflow and tools/assurance/validate_g1_identity_tenant_shell_implementation_scope.py execute from the exact trusted PR base commit. Candidate HEAD is checked out separately as data only. The trusted evaluator consumes exact base/head SHAs plus GitHub-supplied label/repository metadata, loads policy from the exact base commit, evaluates the complete base...head diff with --no-renames and fails closed on any unauthorized path or missing branch/label/claim metadata.
