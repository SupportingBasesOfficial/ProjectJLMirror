...
IMPLEMENTATION DIFF GATE: .github/workflows/g1-identity-tenant-shell-implementation-scope.yml runs in secretless pull_request context. It checks out the exact candidate HEAD with credentials disabled, verifies the exact base object, then materializes the evaluator from the trusted base with git show "${PR_BASE_SHA}:tools/assurance/validate_g1_identity_tenant_shell_implementation_scope.py". Only that temporary base-owned evaluator is executed. It consumes exact base/head SHAs plus GitHub-supplied label/repository metadata, loads policy from the exact base commit, evaluates the complete base...head diff with --no-renames and fails closed on any unauthorized path or missing branch/label/claim metadata. pull_request_target is forbidden by the v1 assurance profile and MUST NOT be used.
AUTHORIZATION-PR BOOTSTRAP: the current authorization PR alone may pass the scope-workflow bootstrap only when base is exactly 7c20c9301711e9a4bf355517d7aa23faad7a05b7, head ref is exactly governance/g1-identity-tenant-shell-authorization, head/base repository are identical, the PR has no jlmirror-slice:g1-identity-tenant-shell label, the HEAD has no implementation/g1-identity-tenant-shell/IMPLEMENTATION_CLAIM.json, and implementation authority remains blocked. Rule: only_when_base_lacks_scope_validator_and_pr_has_no_g1_label_or_implementation_claim. After this authorization is canonical the bootstrap is unreachable because future bases contain the validator and no longer equal the historical bootstrap SHA.
...
RUN: Clean checkout -> create canonical G1 implementation branch -> attach canonical G1 PR label -> create implementation claim -> start G1 dependencies -> migrate/bootstrap -> seed test identity/tenant -> start BFF/backend/frontend -> execute allowed browser journey -> execute forbidden/cross-tenant journey -> container proof -> exact-head CI.
