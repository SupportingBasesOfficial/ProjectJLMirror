...
def configured_policy(policy: dict[str, Any]) -> None:
    expected = {
        ...
        "implementation_claim_authorization_id": EXPECTED_AUTHORIZATION_ID,
        "implementation_pr_base_ref_policy": "must_equal_repository_default_branch",
        "trusted_evaluator_event": "issue_comment",
        "trusted_scope_attestation_command": EXPECTED_ATTESTATION_COMMAND,
        "trusted_scope_readiness_command": EXPECTED_READINESS_COMMAND,
        ...
        "diff_enforcement_rule": EXPECTED_RULE,
        "semantic_guard_rule": EXPECTED_SEMANTIC_RULE,
    }
    for key, value in expected.items():
        if policy.get(key) != value:
            raise AssertionError(f"implementation policy field drift: {key}")
...
def validate_runtime_workflow_text(text: str) -> list[str]:
    ...
    for i, action in enumerate(RUNTIME_ALLOWED_ACTIONS):
        ...
        if not isinstance(step, dict) or set(step) != {"uses"}:
            ...
            continue
        name, sep, sha = step["uses"].partition("@")
        if name != action or not sep or len(sha) != 40 or not all(c in "0123456789abcdefABCDEF" for c in sha):
