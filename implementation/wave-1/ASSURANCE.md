...
- publication-catalog preflight proves only the catalog state observed by the guarded migration; it does **not** claim that `ACCESS EXCLUSIVE` revokes PostgreSQL superuser/equivalent administrative authority or that future publication changes are impossible. Exclusion/segregation of concurrent privileged database administration remains a separately reviewed C2 database-role/operational mapping;
- `FENCE_LOGICAL_REPLICATION_MANIFEST.json` is an exact machine-readable boundary: it must keep the four reuse guards, the fresh all-tables guard, transaction/lock ordering, four forbidden substitutions, Product activation=`none`, Wave 2 authorization=`false`, and the explicit C2 admin non-selection semantics;
...
- fresh bootstrap with `pg_publication` redirected, `puballtables` inverted, the guard moved after first persistent CREATE, or publication tokens present only in comments -> validator finding;
...
- reuse publication checks with catalog/target/predicate inversion, missing version-tolerant `pg_publication_namespace` lookup, wrong `pnnspid` target, wrong `USING v_schema`, moved-after-mutation ordering, or comment laundering -> validator finding;
- `FENCE_LOGICAL_REPLICATION_MANIFEST.json` dropping any required guard/forbidden substitution, claiming permanent admin absence, selecting concurrent-superuser exclusion, enabling Product activation or setting `wave_2_authorized=true` -> validator/test finding;
