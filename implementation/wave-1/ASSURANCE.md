...
- ACL cleanliness and inbound-writer absence never prove outbound logical-replication disclosure absence: fresh bootstrap rejects any existing `FOR ALL TABLES` publication before its first persistent authority-object CREATE, and reuse admission rejects explicit `pg_publication_rel` membership, any `pg_publication.puballtables`, and schema publication through `pg_publication_namespace` where that catalog is supported, all before canonical mutation;
- publication predicates are assurance syntax, not token-presence evidence: the logical-replication validator tokenizes executable SQL/PLpgSQL, distinguishes ordinary string literals from executable predicates, and reconstructs the dynamic schema-publication query before accepting its namespace binding; comments or dead string literals cannot satisfy a guard;
- reused publication preflight control flow is fail-closed as a whole: **no executable `RETURN` or `EXIT` is permitted at any control depth or labeled/plain block ancestry** before the canonical mutation outside the DO block; absence of a depth-zero `RETURN` is not evidence that nested/labeled early exits are absent;
- `RETURN`/`EXIT` text inside ordinary or dollar-quoted literals remains data and cannot be confused with executable early-exit authority;
- publication-catalog preflight proves only the catalog state observed by the guarded migration; it does **not** claim that `ACCESS EXCLUSIVE` revokes PostgreSQL superuser/equivalent administrative authority or that future publication changes are impossible. Exclusion/segregation of concurrent privileged database administration remains the explicitly unselected C2 choice `database_admin_role_and_operational_mapping`;
- `FENCE_LOGICAL_REPLICATION_MANIFEST.json` is an exact machine-readable boundary: it must keep the four reuse guards, the fresh all-tables guard, transaction/lock ordering, four forbidden substitutions, Product activation=`none`, Wave 2 authorization=`false`, and `c2_database_admin_boundary.choice_id=database_admin_role_and_operational_mapping` with explicit non-selection semantics;
- `IMPLEMENTATION_MANIFEST.json.residual_c2_choices_not_selected` must contain that same `database_admin_role_and_operational_mapping` ID; a boundary that exists only in prose is incomplete C2 inventory evidence;
...
- fresh bootstrap with `pg_publication` redirected, `puballtables` inverted, the guard moved after first persistent CREATE, or publication syntax present only in comments/ordinary string literals -> validator finding;
- reused fence table with explicit `pg_publication_rel` membership, any `FOR ALL TABLES` publication, or canonical `platform` schema publication -> revalidation failure before canonical mutation;
- reuse publication checks with catalog/target/predicate inversion, missing version-tolerant `pg_publication_namespace` lookup, wrong `pnnspid` target, wrong `USING v_schema`, moved-after-mutation ordering, comment laundering or dead-string laundering -> validator finding;
- nested `IF true THEN RETURN; END IF;` before required reuse publication guards -> validator finding, regardless of numeric control depth;
- labeled/plain-block `EXIT label` capable of skipping required reuse publication guards -> validator finding;
- dead ordinary/dollar-quoted text containing `RETURN` or `EXIT` must remain non-executable and cannot satisfy or trigger the early-exit guard;
- `FENCE_LOGICAL_REPLICATION_MANIFEST.json` dropping any required guard/forbidden substitution/C2 choice ID, claiming permanent admin absence, selecting concurrent-superuser exclusion, enabling Product activation or setting `wave_2_authorized=true` -> validator/test finding;
- removing `database_admin_role_and_operational_mapping` from the Wave 1 residual-C2 inventory -> validator/test finding;
