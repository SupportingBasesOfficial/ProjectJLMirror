...
Direct ACL cleanliness is also not enough: before C2 role mapping, the migration-owner role must have no direct or transitive `pg_auth_members` path by which another role can assume or inherit owner authority, and non-owner roles cannot retain predefined all-data reachability or column-level fence privileges. This conservative check does not claim to remove PostgreSQL cluster-superuser power; it closes modeled role/ACL laundering inside the Wave 1 boundary.
