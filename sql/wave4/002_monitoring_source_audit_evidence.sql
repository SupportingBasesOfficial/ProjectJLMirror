...
-- Final bounded signature for the source-foundation slice.
...
'Append-only local accountability evidence for privileged Monitoring source mutations. Provider URLs, credentials and native scope are intentionally excluded.';
...
'Atomic create-or-observe transaction. New local source, tenant binding, source generation, durable sync responsibility and privileged audit evidence commit together. ProviderInstance remains referenced lineage, not tenant authority. No provider network call occurs.';
