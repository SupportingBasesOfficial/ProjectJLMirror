-- Wave 4 Monitoring tenant/canonical boundary hardening.
--
-- Successor remediation on top of the shared-provider Monitoring source
-- foundation and the initial-validation worker. This migration adds no new
-- product behavior; it closes persistence-boundary and pooled-tenant gaps.
...
'Deterministic network-free persistence validator for the canonical HTTPS Zabbix endpoint representation admitted by the Monitoring source domain.';
...
CREATE POLICY monitoring_source_tenant_policy ON monitoring.monitoring_source
...
CREATE POLICY monitoring_source_generation_tenant_policy ON monitoring.monitoring_source_generation
USING (tenant_id = NULLIF(current_setting('jlmirror.tenant_id', true), ''))
WITH CHECK (tenant_id = NULLIF(current_setting('jlmirror.tenant_id', true), ''));
...
CREATE POLICY monitoring_sync_operation_tenant_policy ON monitoring.monitoring_sync_operation
USING (tenant_id = NULLIF(current_setting('jlmirror.tenant_id', true), ''))
WITH CHECK (tenant_id = NULLIF(current_setting('jlmirror.tenant_id', true), ''));
...
CREATE POLICY monitoring_source_create_idempotency_tenant_policy ON monitoring.monitoring_source_create_idempotency
USING (tenant_id = NULLIF(current_setting('jlmirror.tenant_id', true), ''))
WITH CHECK (tenant_id = NULLIF(current_setting('jlmirror.tenant_id', true), ''));
...
CREATE POLICY monitoring_source_audit_evidence_tenant_policy ON monitoring.monitoring_source_audit_evidence
USING (tenant_id = NULLIF(current_setting('jlmirror.tenant_id', true), ''))
WITH CHECK (tenant_id = NULLIF(current_setting('jlmirror.tenant_id', true), ''));

ALTER TABLE monitoring.monitoring_source_validation_evidence ENABLE ROW LEVEL SECURITY;
ALTER TABLE monitoring.monitoring_source_validation_evidence FORCE ROW LEVEL SECURITY;
CREATE POLICY monitoring_source_validation_evidence_tenant_policy ON monitoring.monitoring_source_validation_evidence
USING (tenant_id = NULLIF(current_setting('jlmirror.tenant_id', true), ''))
WITH CHECK (tenant_id = NULLIF(current_setting('jlmirror.tenant_id', true), ''));
