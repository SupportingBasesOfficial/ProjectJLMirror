...
-- completion payload is NULL, not an array, above the bounded cardinality, or the
-- caller supplied an incoherent completion shape. Resolve authority first, persist
-- a degraded snapshot, and retire the claim. The v028 implementation remains an
-- executor-only helper for structurally valid completion requests.
...
BEGIN
    ...
    -- Evaluate array cardinality only after proving that the JSON value is an array.
    -- PostgreSQL expression evaluation order must not be relied upon for safety.
    ...
    IF v_input_failure IS NOT NULL THEN
        IF p_metric_definition_snapshot_evidence_id IS NULL
           OR p_metric_definition_snapshot_evidence_id='' THEN
            RAISE EXCEPTION 'monitoring.metric_definition_invalid_snapshot_evidence_id';
        END IF;
    ...
    RETURN monitoring.complete_zabbix_metric_definitions_v028(
        ...
        p_claim_token,
        p_metric_definition_snapshot_evidence_id,
