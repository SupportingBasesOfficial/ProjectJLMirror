-- Wave 4 Health Projection lock-order and supersession hardening.
-- Serialize against Problem State completion through monitoring_source before resource/projection locks.
...
DECLARE
    ...
    v_snapshot_operation_state TEXT;
    v_health TEXT;
    v_evidence TEXT;
...
BEGIN
    IF p_tenant_id IS NULL OR p_tenant_id=''
       OR p_monitoring_resource_id IS NULL OR p_monitoring_resource_id=''
       OR p_health_transition_id IS NULL OR p_health_transition_id=''
       OR length(p_health_transition_id)>512 THEN
        RAISE EXCEPTION 'monitoring.health_projection_invalid_input';
    END IF;

    PERFORM set_config('jlmirror.tenant_id', p_tenant_id, true);

    -- Discovery read grants no authority. It exists only to establish the canonical
    -- source lock key; all resource facts are re-read after source serialization.
    SELECT r.monitoring_source_id
      INTO v_source_id
    ...
    -- Problem State claim/completion also serializes on this source row. Holding it
    -- through Health derivation prevents a later poll generation/snapshot from
    -- superseding the evidence between our authority read and commit.
    SELECT s.active_source_instance_generation,s.configuration_revision,s.scope_revision,
      ...
      INTO v_active_generation,v_configuration_revision,v_scope_revision,
           v_source_evidence,v_problem_poll_epoch,v_problem_poll_generation
      FROM monitoring.monitoring_source AS s
     WHERE s.tenant_id=p_tenant_id
       AND s.monitoring_source_id=v_source_id
     FOR UPDATE;
    IF NOT FOUND THEN
        RAISE EXCEPTION 'monitoring.health_projection_source_not_found';
    END IF;

    SELECT r.source_instance_generation,r.presence_state,r.presence_evidence_state,
           r.scope_state,r.scope_evidence_state,r.scope_projection_revision
      INTO v_generation,v_presence_state,v_presence_evidence,
           v_scope_state,v_scope_evidence,v_scope_projection_revision
      FROM monitoring.monitoring_resource AS r
     WHERE r.tenant_id=p_tenant_id
       AND r.monitoring_resource_id=p_monitoring_resource_id
     ...
     FOR UPDATE;
    IF NOT FOUND THEN
        RAISE EXCEPTION 'monitoring.health_projection_resource_revalidation_failed';
    END IF;
    ...
    SELECT count(*),min(e.problem_snapshot_evidence_id),bool_and(e.snapshot_complete),
           min(e.operational_evidence_state),min(e.operation_state)
      INTO v_snapshot_count,v_snapshot_id,v_snapshot_complete,v_snapshot_state,v_snapshot_operation_state
    ...
    IF EXISTS (
        SELECT 1 FROM monitoring.monitoring_problem AS p
         WHERE p.tenant_id=p_tenant_id AND p.monitoring_source_id=v_source_id
           AND p.source_instance_generation=v_generation
           AND p.monitoring_resource_id=p_monitoring_resource_id
           AND p.problem_state='active' AND p.severity_class='critical'
    ) THEN
        v_health:='unhealthy';
    ELSIF EXISTS (
        SELECT 1 FROM monitoring.monitoring_problem AS p
         WHERE p.tenant_id=p_tenant_id AND p.monitoring_source_id=v_source_id
           AND p.source_instance_generation=v_generation
           AND p.monitoring_resource_id=p_monitoring_resource_id
           AND p.problem_state='active' AND p.severity_class IN ('warning','degraded')
    ) THEN
        v_health:='degraded';
    ELSE
        v_health:='unknown';
    ...
    IF v_source_evidence='current'
       AND v_presence_state='present' AND v_presence_evidence='current'
       AND v_scope_state='in_scope' AND v_scope_evidence='current'
       AND v_scope_projection_revision=v_scope_revision
       AND v_snapshot_count=1 AND v_snapshot_complete
       AND v_snapshot_state='current' AND v_snapshot_operation_state='succeeded' THEN
        v_evidence:='current';
        IF v_health='unknown' AND NOT EXISTS (
            SELECT 1 FROM monitoring.monitoring_problem AS p
             WHERE p.tenant_id=p_tenant_id AND p.monitoring_source_id=v_source_id
               AND p.source_instance_generation=v_generation
               AND p.monitoring_resource_id=p_monitoring_resource_id
               AND p.problem_state='active' AND p.severity_class='unknown'
        ) THEN
            v_health:='healthy';
        END IF;
    ELSE
    ...
    UPDATE monitoring.health_projection
       SET health_class=v_health,evidence_state=v_evidence,projection_revision=v_revision,
           ...
           last_evidence_at=v_now,problem_snapshot_evidence_id=v_snapshot_id,
           reason_refs=v_reason_refs,updated_at=v_now
     WHERE tenant_id=p_tenant_id
       AND monitoring_resource_id=p_monitoring_resource_id
       AND source_instance_generation=v_generation;
