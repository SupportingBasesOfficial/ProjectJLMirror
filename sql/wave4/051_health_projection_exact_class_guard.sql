-- Wave 4 Health Projection exact-class defense-in-depth.
-- A current projection must equal the deterministic class derivable from canonical active problems.
...
BEGIN
    ...
    IF NEW.evidence_state='current' THEN
        ...
        ELSIF EXISTS (
            ...
            v_expected_health:='unknown';
        ELSE
            v_expected_health:='healthy';
        END IF;

        IF NEW.health_class <> v_expected_health THEN
            RAISE EXCEPTION 'Current Health class does not equal canonical Problem State derivation';
        END IF;
    ELSIF NEW.health_class='healthy' THEN
