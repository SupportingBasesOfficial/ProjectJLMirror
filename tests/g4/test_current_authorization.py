...
from current_authorization import BoundMetricsAuthorization  # noqa: E402
...
def load_g1():
    ...
    spec = importlib.util.spec_from_file_location("jlmirror_g1_g4_fixture", path)
...
class AuthorizationTests(unittest.TestCase):
    ...
    def test_metric_and_resource_reads_are_admitted(self):
        for action in ("monitoring.resource.read", "monitoring.metric.read"):
    ...
    def test_other_monitoring_action_is_outside_g4_surface(self):
    ...
    def test_revocation_is_rechecked(self):
        auth = self.auth()
        auth.require(actor_ref="principal-a", tenant_id="tenant-a", action="monitoring.metric.read")
