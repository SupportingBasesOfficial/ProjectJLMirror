...
from tools.authority.fence_sql_contract import validate_fence_sql_text  # noqa: E402
...
class FenceBootstrapSafetyTests(unittest.TestCase):
    ...
    def assert_safety_fails(self, text: str, needle: str | None = None) -> None:
    ...
    def test_transaction_boundaries_fail_closed(self):
        self.assert_safety_fails(self.text().replace("BEGIN;\n", "", 1), "BEGIN")
        self.assert_safety_fails(self.text().replace("\nCOMMIT;\n", "\n", 1), "COMMIT")

    def test_event_trigger_guard_is_exact_and_catalog_bound(self):
        text = self.text()
        self.assert_safety_fails(text.replace("SET LOCAL event_triggers = off;", "SET event_triggers = off;", 1), "SET LOCAL event_triggers")
        self.assert_safety_fails(text.replace("pg_catalog.current_setting('event_triggers')", "current_setting('event_triggers')", 1), "locally disabled")
        self.assert_safety_fails(text.replace("FROM pg_catalog.pg_event_trigger et", "FROM pg_catalog.pg_trigger et", 1), "event trigger")
        self.assert_safety_fails(text.replace("et.evtenabled OPERATOR(pg_catalog.<>) 'D'", "et.evtenabled OPERATOR(pg_catalog.=) 'D'", 1), "event trigger")
    ...
    def test_catalog_bound_expression_primitives_are_required(self):
        text = self.text()
        for old, new in (
            ("pg_catalog.btrim(", "btrim("),
            ("OPERATOR(pg_catalog.~)", "~"),
            ("pg_catalog.statement_timestamp()", "statement_timestamp()"),
        ):
            with self.subTest(old=old):
                self.assert_contract_fails(text.replace(old, new))

    def test_both_fence_functions_pin_pg_catalog_search_path(self):
        text = self.text()
        self.assertEqual(text.count("SET search_path = pg_catalog"), 2)
        self.assert_contract_fails(text.replace("SET search_path = pg_catalog", "SET search_path = public, pg_catalog"))
