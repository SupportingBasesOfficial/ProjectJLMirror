...
EXPECTED_SCOPES = {"unordered", "causal_only", "per_subject_ordered", "per_process_ordered", "per_source_ordered", "custom_bounded_order"}
...
def load_objects() -> tuple[dict, dict, dict, dict]:
    return (json.loads(MANIFEST.read_text()), json.loads(PROFILE.read_text()), json.loads(PLAN.read_text()), json.loads(STATE.read_text()))
...
def validate_objects(source: dict, profile: dict, plan: dict, state: dict) -> list[str]:
    ...
    require(set(source.get("prior_promoted_ledger_credit", [])) == SOURCE_PRIOR_CREDIT, "source historical prior promoted credit drift")
    ...
    require(ceiling_policy.get("definition") == "highest_tested_partition_count_sustaining_at_least_the_tier_specific_target_fraction_and_latency_admission", "partition ceiling policy drift")
    ...
    for scope, mapping in mappings.items():
        ...
        require(mapping.get("serialization") == ("key_serial" if scope in ORDERED_SCOPES else "none"), f"{scope} serialization drift")
    ...
    require(fallback.get("exercise") == "generate_and_route_more_distinct_trusted_logical_scopes_than_the_stress_single_topic_test_ceiling_through_real_kafka_cohort_topics", "fallback must route an actual over-ceiling logical-scope workload")
    ...
    # Current global selection is separate from this historical source-run truth.
    ...
    require(plan.get("selection_state") == "selected", "plan must keep Kafka selected")
    require(plan.get("candidate_status") == "selected_c2_candidate", "plan must keep Kafka as selected C2 candidate")
    require(plan.get("acceptance_state") == "track_selected_separate_d4_acceptance_required", "plan must require separate D4 acceptance")
    completed = d4a.get("evidence_completed", [])
    require(set(completed) == EXPECTED_GLOBAL_CREDIT, "D4-A completed seven-of-seven evidence drift")
    require(len(completed) == len(EXPECTED_GLOBAL_CREDIT), "D4-A completed seven-of-seven evidence multiplicity drift")
