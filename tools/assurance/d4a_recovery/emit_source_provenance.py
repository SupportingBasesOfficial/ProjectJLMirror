...
def main() -> None:
    ...
    assert results["schema"] == "d4a-recovery-benchmark-results-v1"
    assert results["numeric_authority"] == "test_values_only_not_production"
    assert results["real_candidate_outage_exercised"] is True
    assert results["committed_outbox_backlog_survives_broker_outage"] is True
    assert results["priority_preserving_anti_starvation"] is True
    assert results["ack_ambiguity"]["same_logical_identity_reused"] is True
    assert results["broker_progress_non_authority"]["broker_progress_is_business_effect_truth"] is False
    ...
    out = {
        "schema": "d4a-recovery-source-run-provenance-v1",
        "repository_sha": args.repository_sha,
        "workflow_run_id": args.workflow_run_id,
        "workflow_run_attempt": args.workflow_run_attempt,
        "job_id": args.job_id,
        "job_name": args.job_name,
        "probe": "d4a_live_kafka_outbox_recovery",
        ...
        "recovery_profile_sha256": digest(PROFILE),
    ...
    print(
        "d4a_recovery_provenance=PASS "
        f"sha={args.repository_sha} run={args.workflow_run_id} attempt={args.workflow_run_attempt} "
        f"job={args.job_id} index_digest={args.kafka_image_index_digest} amd64_digest={args.kafka_linux_amd64_manifest_digest}"
    )
