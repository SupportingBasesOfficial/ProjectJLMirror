...
def main() -> int:
    ...
    must_fail("premature seventh plan credit", lambda s,p,l,st: l["credited_evidence"].append(EVIDENCE_ID), "global promoted six-of-seven credit drift")
    must_fail("premature seventh state credit", lambda s,p,l,st: next(t for t in st["tracks"] if t["track_id"] == "D4-A")["evidence_completed"].append(EVIDENCE_ID), "state completed six-of-seven evidence drift")
    must_fail("remove recovery remaining", lambda s,p,l,st: next(t for t in st["tracks"] if t["track_id"] == "D4-A")["evidence_remaining"].clear(), "state must leave recovery evidence pending")
    ...
    must_fail("select Kafka", lambda s,p,l,st: s.__setitem__("kafka_selection_state", "selected"), "source package selects Kafka")
    must_fail("grant production", lambda s,p,l,st: s.__setitem__("production_authority", "granted"), "source grants production authority")
    print("d4a_recovery_negative_controls=PASS real_outage=required backlog_durability=required priority_starvation=blocked ack_identity_rewrite=blocked broker_progress_authority=blocked seventh_credit=blocked production_authority=blocked")
    return 0
