...
def validate_objects(source: dict, profile: dict, plan: dict, state: dict) -> list[str]:
    ...
    require(set(d4a.get("evidence_completed", [])) == EXPECTED_GLOBAL_CREDIT, "state completed seven-of-seven evidence drift")
    require(d4a.get("evidence_remaining") == [], "state must have no D4-A evidence remaining after promotion")
