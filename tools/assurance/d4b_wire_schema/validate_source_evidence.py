...
def main(argv: list[str]) -> int:
    ...
    if errors:
        ...
        return 1
    print("d4b_wire_schema_source_manifest=PASS axis=OPEN-EVT-002 concrete_candidates=3 eligible=3 decimal_canonicalization=context_independent canonical_varints=required uint64_varints=bounded protobuf_oneof_duplicates=blocked avro_schema_ref_content=bound avro_historical_schema_digest=sha256_bound avro_writer_fields=required avro_writer_only_fields=validated avro_float_width=ieee754_binary32 avro_float_runtime_mapping=type_strict avro_float_overflow=fail_closed avro_type_resolution=required avro_promotions=reader_canonicalized avro_datum_bounds=required runtime_mapping=bounded historical_binding=required decompression=identity_only equivalent=insufficient_evidence selection=not_selected ledger_credit=0 d4=scoped authorities=not_granted")
    return 0
