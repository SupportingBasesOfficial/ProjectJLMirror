...
def validate(root: Path) -> list[str]:
    ...
    expected_d4b = {'serialization': {'surface_policy': 'explicit_surface_bound_profiles', 'internal_broker': 'protobuf_profile', 'outbound_webhook': 'bounded_json_plus_json_schema_profile'}, 'schema_catalog': 'hybrid_reviewed_git_plus_registry_catalog', 'contract_version': 'positive_integer_family_revision'}
    if ledger.get("candidate") != expected_d4b or ledger.get("candidate_status") != "selected_c2_profile" or ledger.get("selection_state") != "selected":
