...
CURRENT_CREDITS = [
    ...
    D4C_CREDIT_013,D4C_CREDIT_014,D4C_CREDIT_015,D4C_CREDIT_025,
]
...
def _current_errors(state: dict) -> list[str]:
    ...
    d4c = tracks.get("D4-C", {}); d4d = tracks.get("D4-D", {})
    required = d4c.get("required_evidence", []); errors: list[str] = []
    ...
    if d4d.get("evidence_completed") != D4D_CREDITS or d4d.get("evidence_remaining") != [x for x in d4d.get("required_evidence", []) if x not in D4D_CREDITS]: errors.append("D4-D current ledger drift beyond separately promoted OPEN-EVT-016 evidence")
    if sum(len(t.get("evidence_completed", [])) for t in tracks.values()) != 23: errors.append("D4-wide evidence count drift")
    return errors
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print("d4c_ack_lease_checkpoint_source=PASS source_auto_credit=false historical_oracle=preserved current_d4c=9_of_9 current_d4d=2_of_5 d4wide=23_of_26 selection=not_selected")
    return 0
