...
def _current_errors(state: dict) -> list[str]:
    ...
    if d4d.get("evidence_completed")!=D4D_CURRENT or d4d.get("evidence_remaining")!=[x for x in required if x not in D4D_CURRENT]: errors.append("D4-D current state must be exactly 3/5")
    if sum(len(t.get("evidence_completed",[])) for t in tracks.values())!=24: errors.append("D4-wide evidence count must be exactly 24/26")
    return errors
...
def main(argv: list[str]) -> int:
    ...
    if errors:
        ...
        return 1
    print("d4c_open_evt_010_source=PASS historical_current_oracle=byte_preserved current_d4d=3_of_5 current_d4wide=24_of_26")
    return 0
