...
def validate(root: Path) -> list[str]:
    errors: list[str] = []
    try:
        manifest = load(root / MANIFEST); plan = load(root / PLAN); state = load(root / STATE)
    ...
    for key, expected in {
        ...
        "mode": "candidate_source_evidence_only", "selection_state": "not_selected", "selection_authority": "not_granted",
    }.items():
    ...
    if not isinstance(checks, dict) or set(checks) != set(CANDIDATES) or not all(all(v.values()) for v in checks.values()): errors.append("runtime proof checks incomplete")
    tracks_raw = state.get("tracks") if isinstance(state, dict) else None
    if not isinstance(tracks_raw, list) or len(tracks_raw) != 4:
        errors.append("global D4 track structure drift"); return errors
    tracks = {t.get("track_id"): t for t in tracks_raw if isinstance(t, dict)}
    if set(tracks) != {"D4-A", "D4-B", "D4-C", "D4-D"}:
        errors.append("global D4 track identity drift"); return errors
