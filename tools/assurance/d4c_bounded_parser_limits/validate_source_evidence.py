...
def validate(root: Path) -> list[str]:
    ...
    if d4c.get("evidence_completed") != CURRENT_CREDITS or d4c.get("evidence_remaining") != expected_remaining: errors.append("D4-C current 8/9 ledger drift")
    if d4d.get("evidence_completed") != [] or d4d.get("candidate") is not None: errors.append("D4-D state leakage")
    if sum(len(t.get("evidence_completed", [])) for t in tracks_raw) != 20: errors.append("D4-wide evidence count drift")
...
def main(argv: list[str]) -> int:
    ...
    if errors:
        ...
        return 1
    print("d4c_open_evt_010_source=PASS candidates=3 checks=18_of_18 proof_inventory=exact source_snapshot_nonpromoting=true bounded_before_allocation=true malformed_gzip=blocked concatenated_gzip=blocked duplicate_members=blocked parser_nesting_prechecked=true specialized_planes=referenced_at_any_depth deterministic_nonretryable=true fixture_limits_noncanonical=true source_auto_credit=false current_d4c=8_of_9 current_d4wide=20_of_26 selection=not_selected")
    return 0
