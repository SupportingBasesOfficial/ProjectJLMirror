...
def main() -> int:
    ...
    if (
        d4d.get("evidence_completed") != [D4D_CREDIT]
        or d4d.get("evidence_remaining") != [x for x in required_d4d if x != D4D_CREDIT]
        or d4d.get("candidate") is not None
        or d4d.get("candidate_status") != "not_selected"
        or d4d.get("state") != "candidate_selection_open"
    ):
        return fail("D4-D current projection must remain exactly 1/5")
    if sum(len(t.get("evidence_completed", [])) for t in tracks.values()) != 22:
        return fail("D4-wide current credit count must remain 22/26")
    ...
    print(
        ...
        "source_schema=exact source_snapshot=6/9_uncredited current_d4c=9/9 current_d4d=1/5 current_d4wide=22/26 "
