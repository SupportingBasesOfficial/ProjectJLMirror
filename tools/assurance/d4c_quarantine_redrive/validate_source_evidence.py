...
def validate(root: Path) -> list[str]:
    ...
    expected_scalars = {
        ...
        "gate_id": "D4",
        "track_id": "D4-C",
        "axis": "quarantine_and_redrive",
        "source_decision": "OPEN-EVT-009",
        "evidence_id": EVIDENCE,
        "canonical_base": "48734ede4bceb6b4f25f7ac5c9f84ced9563e351",
    ...
    try:
        ...
        axis = {}
        errors.append("accepted OPEN-EVT-009 axis missing")
    if axis.get("decision") != "OPEN-EVT-009" or axis.get("evidence_id") != EVIDENCE:
        errors.append("accepted axis binding drift")
    if source.get("required_proofs") != axis.get("must_prove"):
        errors.append("required proof inventory drift from accepted plan")
    concrete = [c for c in axis.get("candidate_classes", []) if c != "equivalent_reviewed_profile"]
    if concrete != list(CANDIDATES):
        errors.append("candidate class inventory drift from accepted plan")
    if source.get("source_assertions") != EXPECTED_ASSERTIONS:
        errors.append("source assertion inventory drift")
    if source.get("non_authority") != EXPECTED_NON_AUTHORITY:
        errors.append("source non-authority boundary drift")

    runtime = evaluate_all()
    if source.get("candidate_results") != runtime.get("candidate_results"):
        errors.append("candidate results drift from executable harness")
    if source.get("equivalent_reviewed_profile") != "insufficient_evidence" or runtime.get("equivalent_reviewed_profile") != "insufficient_evidence":
    ...
    if runtime.get("selection") != "not_selected" or runtime.get("ledger_credit") != []:
        errors.append("runtime escaped source-only boundary")
    if not all(all(checks.values()) for checks in runtime.get("checks", {}).values()):
