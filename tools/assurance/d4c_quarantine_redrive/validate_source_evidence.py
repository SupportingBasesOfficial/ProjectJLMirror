...
def _current_errors(state: dict) -> list[str]:
    ...
    if state.get("gate_state") != "separately_accepted": errors.append("global authority drift: gate_state")
    for field, expected in {
        ...
        if state.get(field) != expected: errors.append(f"global authority drift: {field}")
    return errors
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print("d4c_quarantine_redrive_source=PASS historical_gate=scoped historical_oracle=preserved current_gate=separately_accepted current_d4c=9_of_9_selected current_d4d=5_of_5_selected current_d4wide=26_of_26 authorities=unchanged")
    return 0
