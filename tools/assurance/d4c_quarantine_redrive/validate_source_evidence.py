...
def _current_errors(state: dict) -> list[str]:
    ...
    if d4d.get("evidence_completed") != D4D_CREDITS or d4d.get("evidence_remaining") != [x for x in required if x not in D4D_CREDITS]:
        errors.append("D4-D current state must be exactly 2/5")
    if sum(len(t.get("evidence_completed", [])) for t in tracks.values()) != 23:
        errors.append("D4-wide evidence count must be 23/26")
    return errors
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print("d4c_quarantine_redrive_source=PASS historical_oracle=preserved current_d4c=9_of_9 current_d4d=2_of_5 current_d4wide=23_of_26 selection=not_selected")
    return 0
