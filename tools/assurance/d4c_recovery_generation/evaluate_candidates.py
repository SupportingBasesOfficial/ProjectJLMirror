...
class Evidence:
    ...
    identity: str
    semantic_digest: str
    scope: str = "tenant-a/orders"
    reproduction_authority: str = "none"
...
class RecoveryManifest:
    ...
    @classmethod
    def from_durable_json(cls, raw: str) -> "RecoveryManifest":
    ...
    def reconcile(self, manifest: RecoveryManifest, inventory: dict[str, Evidence], *, current_generation: int, target_scope: str = "tenant-a/orders") -> dict[str, Any]:
        ...
        for name in self.REQUIRED:
            ...
            if item.scope != target_scope:
                raise ContractViolation(f"inventory_scope_mismatch:{name}")
        ...
        if webhook.semantic_digest != eq.semantic_digest and webhook.reproduction_authority != "governed_reproduction":
            raise ContractViolation("webhook_semantic_recovery_unavailable")
...
def check_candidate(candidate: str) -> dict[str, bool]:
    ...
    durable_manifest = RecoveryManifest.from_durable_json(manifest.durable_json())
    ...
    wrong_scope_verifier = with_item(inventory, "equivalence", Evidence("equivalence", 9, "present", "eq-001", "sem-001", scope="tenant-b/orders"))
    bad_webhook = with_item(inventory, "webhook_delivery", Evidence("webhook_delivery", 9, "present", "delivery-001", "different-semantic"))
    ...
    return {
        ...
        "manifest_durable_roundtrip": durable_manifest == manifest and durable_manifest.fence_generation == 9,
        ...
        "webhook_semantic_recovery_proven": result["semantic_digest"] == inventory["webhook_delivery"].semantic_digest,
        "webhook_semantic_drift_blocks": blocked(lambda: reconciler.reconcile(manifest, bad_webhook, current_generation=9), "webhook_semantic_recovery_unavailable"),
        ...
        "cross_scope_verifier_blocks": blocked(lambda: reconciler.reconcile(manifest, wrong_scope_verifier, current_generation=9), "inventory_scope_mismatch:equivalence"),
...
PROOF_CHECKS = {
    PROOFS[0]: ("generation_and_fence_explicit", "manifest_durable_roundtrip"),
    PROOFS[1]: ("rf_inventory_complete",),
    PROOFS[2]: ("webhook_identity_stable", "webhook_semantic_recovery_proven", "webhook_semantic_drift_blocks", "stale_destination_blocks"),
    PROOFS[3]: ("missing_state_blocks",),
    PROOFS[4]: ("older_equivalence_blocks", "unverifiable_equivalence_blocks"),
    PROOFS[5]: ("obsolete_verifier_blocks", "obsolete_profile_blocks", "cross_scope_verifier_blocks"),
    ...
    PROOFS[8]: ("obsolete_verifier_blocks", "obsolete_profile_blocks", "cross_scope_verifier_blocks"),
