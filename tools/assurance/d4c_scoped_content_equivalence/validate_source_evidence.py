...
def main() -> int:
    ...
    require(ledger["ledger_credit_state"] == "nine_of_nine", "current D4-C ledger must be 9/9 after OPEN-EVT-025 promotion")
    ...
    require(ledger["remaining_evidence"] == [], "D4-C remaining count drift")
    ...
    require(d4c["evidence_remaining"] == [], "state/ledger D4-C mismatch")
    ...
    require(sum(len(track["evidence_completed"]) for track in state["tracks"]) == 21, "D4-wide must be 21/26")
    ...
    print("d4c_open_evt_011_source_validation=PASS source_snapshot=3_of_9_unchanged proof_map=15_of_15 fingerprint=ineligible_only_by_confidentiality keyed_digest=eligible retained_original=eligible hybrid=eligible current_d4c=9/9 d4wide=21/26 selection=none authorities=unchanged")
    return 0
