...
def main() -> int:
    ...
    require(ledger["ledger_credit_state"] == "eight_of_nine", "current D4-C ledger must be 8/9 after OPEN-EVT-015 promotion")
    ...
    require(len(ledger["remaining_evidence"]) == 1, "D4-C remaining count drift")
    ...
    require(sum(len(track["evidence_completed"]) for track in state["tracks"]) == 20, "D4-wide must be 20/26")
    ...
    print("d4c_open_evt_011_source_validation=PASS source_snapshot=3_of_9_unchanged proof_map=15_of_15 fingerprint=ineligible_only_by_confidentiality keyed_digest=eligible retained_original=eligible hybrid=eligible current_d4c=8/9 d4wide=20/26 selection=none authorities=unchanged")
    return 0
