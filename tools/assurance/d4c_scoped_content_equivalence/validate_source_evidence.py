    ...
    if results is not None:
        ...
        for candidate, status in EXPECTED_CANDIDATES.items():
            ...
            for proof, check_names in PROOF_TO_CHECKS.items():
                ...
                if status == "eligible_for_evidence_execution":
                    require(proven, f"eligible candidate lacks proof {proof}: {candidate}")
                elif candidate == fingerprint and proof == confidentiality_proof:
