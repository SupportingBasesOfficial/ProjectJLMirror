...
SENSITIVE_KEYS = {"authorization", "cookie", "set-cookie", "secret", "password", "credential", "api_key", "token"}
...
def _redact_attributes(attributes: dict[str, str]) -> tuple[tuple[str, str], ...]:
    ...
    for key, value in attributes.items():
        if not key or len(key) > MAX_ATTR_KEY or len(value) > MAX_ATTR_VALUE:
            raise TraceContextRejected("trace attribute out of bounds")
        lowered = key.lower()
        clean.append((key, "[REDACTED]" if lowered in SENSITIVE_KEYS else value))
...
def normalize_trace_context(traceparent: str | None, tracestate: str | None, attributes: dict[str, str] | None = None) -> TraceContext:
    ...
    state = _bounded_tracestate(tracestate)
    if traceparent is None:
        return TraceContext(None, state, attrs)
    if not _valid_traceparent(traceparent):
        raise TraceContextRejected("malformed traceparent")
    return TraceContext(traceparent, state, attrs)
...
def business_identity(observed: ObservedMessage) -> tuple[str, str, str, str, str]:
    e = observed.envelope
    return (e.tenant_id, e.message_id, e.idempotency_key, e.ordering_key, e.payload)
...
def run_probes() -> dict[str, bool]:
    env = BusinessEnvelope("tenant-a", "msg-1", "idem-1", "order-1", "payload-v1")
    valid = "00-4bf92f3577b34da6a3ce929d0e0e4736-00f067aa0ba902b7-01"
    same_trace_other_parent = "00-4bf92f3577b34da6a3ce929d0e0e4736-1111111111111111-01"

    observed = process_message(env, traceparent=valid, tracestate="vendor=value", attributes={"component": "consumer", "authorization": "Bearer super-secret"})
    no_trace = process_message(env, traceparent=None)

    checks: dict[str, bool] = {
        "trace_context_is_observability_only": business_identity(observed) == business_identity(no_trace),
        ...
        "sensitive_trace_attribute_redacted": dict(observed.trace.attributes)["authorization"] == "[REDACTED]",
        "non_sensitive_trace_attribute_preserved": dict(observed.trace.attributes)["component"] == "consumer",
        "missing_trace_context_preserves_business_semantics": business_identity(no_trace) == business_identity(observed),
    }
    ...
    tenant_a_2 = process_message(BusinessEnvelope("tenant-a", "msg-2", "idem-2", "order-2", "payload-v2"), traceparent=same_trace_other_parent)
    tenant_b = process_message(BusinessEnvelope("tenant-b", "msg-3", "idem-3", "order-3", "payload-v3"), traceparent=same_trace_other_parent)
    ...
    checks["trace_change_does_not_change_business_effect"] = business_identity(altered_trace) == business_identity(observed)
