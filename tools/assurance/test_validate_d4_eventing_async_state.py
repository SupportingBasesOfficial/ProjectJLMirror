...
def main() -> int:
    ...
    if validator.EXPECTED_TOTAL_CREDITED != 16:
    ...
    def regress_fourth_credit(s):
        d = d4c(s)
        credit = "scoped_content_equivalence_confidentiality_and_conflict_rejection"
        d["evidence_completed"].remove(credit)
        d["evidence_remaining"].insert(0, credit)
    must_fail(regress_fourth_credit, "completed evidence drift")

    def leak_fifth_credit(s):
        d = d4c(s)
        d["evidence_completed"].append(d["evidence_remaining"].pop(0))
    must_fail(leak_fifth_credit, "completed evidence drift")
    ...
    print("d4_state_falsification=PASS full_d4_acceptance=blocked transport_grant=blocked d4a_b_regression=blocked d4c_fourth_credit_regression=blocked d4c_fifth_credit_leakage=blocked d4c_selection_leakage=blocked duplicate_track_identity=blocked product_wave4_production=blocked c3_scope_leak=blocked total_required=26 total_credited=16")
    return 0
