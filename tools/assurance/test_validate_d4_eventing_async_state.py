...
def main():
 ...
 must_fail(lambda s:track(s,'D4-C').__setitem__('candidate_status','not_selected'),'D4-C selected profile status drift')
 must_fail(lambda s:track(s,'D4-C').__setitem__('state','candidate_selection_open'),'D4-C selected state drift')
 def drift_recovery(s):
  track(s,'D4-C')['candidate']['recovery_generation_reconciliation_and_activation']='restore_generation_fence_manifest_profile'
 must_fail(drift_recovery,'D4-C selected profile drift')
 ...
 print('d4_state_falsification=PASS total_required=26 total_credited=26 d4c_selection=bound d4d_selection=bound credit_regressions=blocked authorities=blocked separate_acceptance=preserved')
 return 0
