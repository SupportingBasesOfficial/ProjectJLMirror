...
def main() -> int:
    ...
    if validator.EXPECTED_TOTAL_CREDITED != 6:
    ...
    must_fail(lambda s: d4a(s)["evidence_completed"].append("broker_outbox_dispatch_priority_preserving_backlog_drain_recovery_benchmark"), "completed evidence drift")
    must_fail(lambda s: d4a(s)["evidence_completed"].pop(), "completed evidence drift")
    must_fail(lambda s: d4a(s)["evidence_remaining"].append("capacity_envelope_baseline_growth_stress"), "remaining evidence drift")
    must_fail(lambda s: d4a(s)["evidence_remaining"].clear(), "remaining evidence drift")
    ...
    print("d4_state_falsification=PASS premature_acceptance=blocked authority_escalation=blocked unauthorized_seventh_credit=blocked credit_removal=blocked candidate_leak=blocked c3_scope_leak=blocked total_required=26 total_credited=6")
    return 0
