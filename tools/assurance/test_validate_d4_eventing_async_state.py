...
def main():
 ...
 if errors: raise AssertionError(f'canonical D4 accepted gate state failed validation: {errors!r}')
 ...
 for tid in ['D4-A','D4-B','D4-C','D4-D']:
  must_fail(lambda s,t=tid:track(s,t).__setitem__('state','accepted_candidate'),tid+' terminal selected state drift')
  must_fail(lambda s,t=tid:track(s,t)['evidence_completed'].pop(),tid+' completed evidence drift')
 must_fail(lambda s:track(s,'D4-A').__setitem__('candidate','rabbitmq'),'D4-A selected candidate drift')
 must_fail(lambda s:track(s,'D4-B').__setitem__('candidate',None),'D4-B selected profile drift')
 must_fail(lambda s:track(s,'D4-C').__setitem__('candidate',None),'D4-C selected profile drift')
 must_fail(lambda s:track(s,'D4-D').__setitem__('candidate',None),'D4-D selected profile drift')
 ...
 print('d4_acceptance_falsification=PASS evidence=26_of_26 tracks=terminal_selected gate=separately_accepted transport_implementation_authority=blocked product_wave4=blocked production=blocked c3=blocked')
 return 0
