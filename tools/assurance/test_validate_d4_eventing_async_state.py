...
def main():
 ...
 if validator.EXPECTED_TOTAL_EVIDENCE!=26 or validator.EXPECTED_TOTAL_CREDITED!=23: raise AssertionError('unexpected D4 evidence totals')
 ...
 first,second,third=validator.EXPECTED_REQUIRED_EVIDENCE['D4-D'][:3]
 def regress_credit(s,credit):
  d=track(s,'D4-D'); d['evidence_completed'].remove(credit); d['evidence_remaining'].insert(0,credit)
 must_fail(lambda s:regress_credit(s,first),'completed evidence drift')
 must_fail(lambda s:regress_credit(s,second),'completed evidence drift')
 must_fail(lambda s:track(s,'D4-D')['evidence_completed'].append(first),'completed evidence drift')
 def grant_third(s):
  d=track(s,'D4-D'); d['evidence_remaining'].remove(third); d['evidence_completed'].append(third)
 must_fail(grant_third,'completed evidence drift')
 ...
 print('d4_state_falsification=PASS total_required=26 total_credited=23 d4d_first_second_regression=blocked duplicate=blocked third_credit_without_promotion=blocked selection=blocked authorities=blocked')
 return 0
