...
def main() -> int:
    ...
    if validator.EXPECTED_TOTAL_CREDITED != 15:
    ...
    def regress_third_credit(s):
        d = d4c(s)
        credit = "bounded_message_batch_compression_and_parser_limits"
        d["evidence_completed"].remove(credit)
        d["evidence_remaining"].insert(0, credit)
    must_fail(regress_third_credit, "completed evidence drift")

    def leak_fourth_credit(s):
        d = d4c(s)
        d["evidence_completed"].append(d["evidence_remaining"].pop(0))
    must_fail(leak_fourth_credit, "completed evidence drift")
    ...
    print("d4_state_falsification=PASS full_d4_acceptance=blocked transport_grant=blocked d4a_b_regression=blocked d4c_third_credit_regression=blocked d4c_fourth_credit_leakage=blocked d4c_selection_leakage=blocked duplicate_track_identity=blocked product_wave4_production=blocked c3_scope_leak=blocked total_required=26 total_credited=15")
    return 0
