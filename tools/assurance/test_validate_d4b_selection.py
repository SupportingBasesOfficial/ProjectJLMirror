...
def baseline() -> dict[Path, dict]:
    return {path: json.loads((ROOT / path).read_text(encoding="utf-8")) for path in FILES}


def write_fixture(root: Path, documents: dict[Path, dict]) -> None:
