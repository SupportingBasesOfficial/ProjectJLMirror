...
def add_unpromoted_d4d_third_credit(data: dict[Path, object]) -> None:
    d4d = track(data, "D4-D")
    third = d4d["evidence_remaining"][0]
    d4d["evidence_completed"].append(third)
    d4d["evidence_remaining"].remove(third)
...
def main() -> int:
    ...
    must_fail(add_unpromoted_d4d_third_credit, "D4-D current evidence must be exactly 2/5")
    ...
    print("d4c_candidate_evaluation_falsification=PASS duplicate_json=blocked hidden_selection=blocked axis_removal=blocked hidden_preference=blocked candidate_collapse=blocked non_string_collection=blocked proof_removal=blocked proof_substitution=blocked fixed_phase10_proof_omissions=blocked evidence_binding_drift=blocked source_decision_drift=blocked output_escalation=blocked d4a_d4b_regression=blocked d4c_credit_leak=blocked d4d_third_credit_without_promotion=blocked authority_escalation=blocked")
    return 0
