...
def main() -> int:
    ...
    semantic_cases = (
        ...
        ("apps/g2-monitoring-source-onboarding/source.ts", "await database[`insert`]({sourceId, value});"),
        ("apps/g2-monitoring-source-onboarding/source.ts", 'return reply.send({value: payload["password"]});'),
        ...
        ("apps/g2-monitoring-source-onboarding/source.ts", "let out; out = (\n payload.password\n); reply.send({value: out});"),
        ("apps/g2-monitoring-source-onboarding/source.ts", 'Reflect.apply((reply /* harmless */)["send"], reply, [{value: payload.password}]);'),
        ("apps/g2-monitoring-source-onboarding/source.ts", "const ddl = `GRANT TEMP ON DATABASE app TO role`;"),
        ...
        ("apps/g2-monitoring-source-onboarding/source.ts", 'let out; out = typeof /[//]/.test(input) ? payload.password : ""; reply.send({value: out});'),
        ("apps/g2-monitoring-source-onboarding/source.ts", "reply.send.call?.(reply, {value: payload.password});"),
        ("apps/g2-monitoring-source-onboarding/source.ts", "reply.send.apply?.(reply, [{value: payload.password}]);"),
        ("apps/g2-monitoring-source-onboarding/source.ts", 'reply?.["send"]?.({value: payload.password});'),
        ("apps/g2-monitoring-source-onboarding/source.ts", 'db.query(Prisma["sql"]`GRANT SELECT ON TABLE monitoring.sources TO app`);'),
