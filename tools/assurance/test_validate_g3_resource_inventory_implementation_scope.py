...
def main() -> int:
    ...
    run_case({"apps/g3-resource-inventory/alias_write.py":"writer = db\nwriter.update('monitoring_resource', {'x': 1})\n"}, expect_ok=False)
    run_case({"apps/g3-resource-inventory/self_alias.py":"writer = self.db\nwriter['delete']('monitoring_resource')\n"}, expect_ok=False)
    run_case({"apps/g3-resource-inventory/bound_write.py":"write = db.update\nwrite('monitoring_resource', {'x': 1})\n"}, expect_ok=False)
    run_case({"apps/g3-resource-inventory/reflection.py":"write = getattr(db, 'upsert')\nwrite('monitoring_resource')\n"}, expect_ok=False)
    ...
    run_case({"apps/g3-resource-inventory/ui.py":"message = 'Update inventory view'\n"}, expect_ok=True)
    ...
    run_case({".github/workflows/g3-resource-inventory-runtime.yml":"{\"name\":\"JLMIRROR G3 Resource Inventory Runtime\",\"on\":{\"pull_request\":{},\"workflow_dispatch\":{}},\"permissions\":{},\"jobs\":{\"g3-runtime\":{\"runs-on\":\"ubuntu-24.04\",\"steps\":[{\"uses\":\"actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1\",\"with\":{\"repository\":\"attacker/repo\",\"path\":\"tools/g3\"}},{\"uses\":\"actions/setup-python@ece7cb06caefa5fff74198d8649806c4678c61a1\"},{\"uses\":\"actions/setup-node@249970729cb0ef3589644e2896645e5dc5ba9c38\"},{\"run\":\"python tools/g3/run_resource_inventory_runtime.py\"}]}}}"}, expect_ok=False)
    run_rename_case()
