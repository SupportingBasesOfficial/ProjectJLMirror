...
def _validate_head_status_workflow(root: Path) -> list[str]:
    ...
    text = path.read_text(encoding='utf-8')
    markers = {
        'issue_comment trigger': 'issue_comment:', 'per-PR concurrency': 'group: adversarial-learning-${{ github.event.issue.number }}',
        'superseded-run cancellation': 'cancel-in-progress: true', 'zero workflow-level permissions': 'permissions: {}',
        'trusted resolve job': '  resolve:', 'isolated pending publisher': '  publish-pending:', 'read-only analysis job': '  analyze:',
        'isolated final publisher': '  publish-final:', 'resolved PR head lookup': 'pulls/${PR_NUMBER}',
        'head status endpoint': 'statuses/${PR_HEAD_SHA}', 'stable status context': 'JLMIRROR / adversarial-learning-reconciliation',
...
def validate(root: Path, review_comments: Path | None = None) -> list[str]:
    root=root.resolve(); errors=list(base.validate(root,None))
    errors.extend(_validate_head_status_workflow(root)); errors.extend(_validate_d4c_current_workflow_projections(root)); errors.extend(_validate_falsifier_effects(root))
...
def main() -> None:
    parser=argparse.ArgumentParser(); parser.add_argument('--root',type=Path,default=Path.cwd()); parser.add_argument('--review-comments',type=Path); args=parser.parse_args()
