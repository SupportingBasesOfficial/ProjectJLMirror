...
def validate_manifest(state):
 ...
 req(state.get('gate_state')=='scoped','D4 must remain scoped'); req(state.get('d4_transport_authority')=='selected_not_granted','transport authority drift'); req(state.get('canonical_product_implementation_authority')=='not_granted','product authority drift'); req(state.get('wave4_implementation_authority')=='not_granted','Wave4 authority drift'); req(state.get('production_authority')=='none','production authority drift'); req(state.get('c3_numeric_topology_authority')=='not_selected','C3 authority drift')
 ...
 for tid,t in [('D4-C',c),('D4-D',d)]: req(t.get('candidate') is None and t.get('candidate_status')=='not_selected' and t.get('state')=='candidate_selection_open',tid+' open/unselected drift')
 req(sum(len(t.get('required_evidence',[])) for t in tracks)==26,'D4 required evidence total drift'); req(sum(len(t.get('evidence_completed',[])) for t in tracks)==23,'D4 credited evidence total drift'); req(set(state.get('explicit_c3_exclusions',[]))==EXPECTED_C3_EXCLUSIONS,'D4 C3 exclusion set drift'); req(set(state.get('explicit_product_or_later_gate_exclusions',[]))==EXPECTED_LATER_EXCLUSIONS,'D4 Product/later exclusion drift'); req('separate_acceptance' in state.get('acceptance_rule',''),'D4 acceptance must remain separate'); req('separate_explicit_user_authorization' in state.get('merge_rule',''),'D4 merge must remain separately authorized')
 return e
