...
EXPECTED_COMPLETED={k:list(v) for k,v in EXPECTED_REQUIRED_EVIDENCE.items()}
...
EXPECTED_D4D_CANDIDATE={'workload_identity_to_broker_credential_adapter':'derived_short_lived_broker_native_credential_adapter','tenant_and_contract_scoped_producer_consumer_authorization':'broker_acl_projection_adapter','message_protection_key_authority_and_historical_verifier_continuity':'kms_backed_envelope_or_transport_protection_profile','secret_credential_payload_exclusion_and_erasure_boundary':'reference_only_secret_authority_profile','trace_context_observability_only_validation_and_redaction':'w3c_trace_context_bounded_profile'}
...
def validate_manifest(state):
 ...
 for tid in ['D4-A','D4-B','D4-C','D4-D']:
  t=by[tid]; r=EXPECTED_REQUIRED_EVIDENCE[tid]; c=EXPECTED_COMPLETED[tid]
  req(t.get('source_decisions')==EXPECTED_TRACK_SOURCES[tid],tid+' source decision drift'); req(t.get('required_evidence')==r,tid+' required evidence drift'); req(t.get('evidence_completed')==c,tid+' completed evidence drift'); req(t.get('evidence_remaining')==[],tid+' remaining evidence drift'); req(len(c)==len(set(c)),tid+' duplicate completed evidence')
 ...
 req(c.get('candidate') is None,'D4-C must not silently select a candidate'); req(c.get('candidate_status')=='not_selected','D4-C candidate status must remain not_selected'); req(c.get('state')=='candidate_selection_open','D4-C scoped state drift')
 req(d.get('candidate')==EXPECTED_D4D_CANDIDATE,'D4-D selected profile drift'); req(d.get('candidate_status')=='selected_c2_security_profile','D4-D selected profile status drift'); req(d.get('state')=='selected_candidate','D4-D selected state drift')
...
if __name__=='__main__':
 ...
 print('d4_eventing_async_state=PASS evidence_required=26 evidence_credited=26 d4a=7_of_7_selected d4b=5_of_5_selected d4c=9_of_9_unselected d4d=5_of_5_selected d4=scoped acceptance=separate authorities=unchanged')
