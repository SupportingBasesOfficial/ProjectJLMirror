...
def validate(root: Path) -> list[str]:
    ...
    for evidence_id, required in REQUIRED_ASSERTIONS.items():
        assertions = set(by_id.get(evidence_id, {}).get("must_prove", []))
        require(required <= assertions, f"{evidence_id} missing binding proof assertions")
        require(by_id.get(evidence_id, {}).get("evidence_kind") not in (None, "documentation_only"), f"{evidence_id} evidence kind invalid")
