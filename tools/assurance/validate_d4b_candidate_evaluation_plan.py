...
EXPECTED_PLAN_KEYS = {
    "schema_version", "gate_id", "track_id", "canonical_base", "mode",
    "selection_state", "selection_authority", "separate_selection_required",
    "separate_d4_acceptance_required", "source_decisions", "axes",
    "cross_axis_invariants", "evaluation_output_states", "forbidden_outputs",
}
EXPECTED_AXES = {
...
def validate(root: Path) -> list[str]:
    ...
    cross = plan.get("cross_axis_invariants", [])
    req(isinstance(cross, list) and set(cross) == EXPECTED_CROSS and len(cross) == len(EXPECTED_CROSS), "cross-axis exact invariant inventory drift")
    outputs = plan.get("evaluation_output_states", [])
    req(isinstance(outputs, list) and set(outputs) == EXPECTED_OUTPUTS and len(outputs) == len(EXPECTED_OUTPUTS), "evaluation output state inventory drift")
    forbidden = plan.get("forbidden_outputs", [])
    req(isinstance(forbidden, list) and set(forbidden) == EXPECTED_FORBIDDEN and len(forbidden) == len(EXPECTED_FORBIDDEN), "forbidden output inventory drift")
    ...
    if isinstance(tracks, list) and len(tracks) == 4 and all(isinstance(t, dict) for t in tracks):
        ...
        if len(ids) == len(set(ids)) and set(ids) == {"D4-A", "D4-B", "D4-C", "D4-D"}:
            ...
            req(d4b.get("state") == "evidence_complete_selection_pending" and set(d4b.get("evidence_completed", [])) == EXPECTED_D4B_EVIDENCE and len(d4b.get("evidence_completed", [])) == 5, "D4-B state/credit drift")
...
def main(argv: list[str]) -> int:
    ...
    if errors:
        ...
        return 1
    print("d4b_candidate_evaluation_plan=PASS axes=3 exact_schema=true strict_json=true selection=not_selected d4b=5_of_5 d4a=kafka_7_of_7 d4=scoped authorities=not_granted")
    return 0
