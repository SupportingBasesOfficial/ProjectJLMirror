...
def validate(root: Path) -> list[str]:
    ...
    req(promotion.get("source_review", {}).get("review_id") == 5108877160, "source review provenance drift")
    req(promotion.get("source_review", {}).get("material_threads_unresolved") == 0, "source review must have zero unresolved material threads")
    ...
    req(set(promotion.get("credited_evidence", [])) == EXPECTED_IDS and promotion.get("credit_count") == 5, "promotion credit set drift")
    ...
    req(promotion.get("source_manifest", {}).get("sha256") == EXPECTED_MANIFEST_SHA256, "promotion source-manifest digest drift")
    ...
    req(tracks["D4-A"].get("candidate") == "kafka" and len(tracks["D4-A"].get("evidence_completed", [])) == 7, "D4-A selected 7/7 state drift")
    for sibling in (tracks["D4-C"], tracks["D4-D"]):
...
def main(argv: list[str]) -> int:
    ...
    if errors:
        ...
        return 1
    print("d4b_ledger_promotion=PASS credited=5/5 candidate=not_selected selection=separate source_immutable=true d4=scoped authorities=not_granted")
    return 0
