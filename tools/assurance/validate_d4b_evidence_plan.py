...
def _current_total_errors(state: dict) -> list[str]:
    ...
    if sum(len(t.get("evidence_completed", [])) for t in tracks) != 24:
        return ["D4-wide current evidence must be exactly 24/26"]
    return []
...
def main(argv: list[str]) -> int:
    ...
    if errors:
        ...
        return 1
    print("d4b_evidence_plan=PASS historical_oracle=preserved current_sibling_d4c=9_of_9 current_sibling_d4d=3_of_5 d4wide=24_of_26")
    return 0
