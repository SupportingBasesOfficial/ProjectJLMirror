...
def validate_authorities(record,prefix,gate_field):
 e=[]
 def req(ok,msg):
  if not ok:e.append(prefix+msg)
 req(record.get(gate_field)=='scoped','D4 gate authority escalation')
...
def validate_records(selection,ledger,state,evaluation):
 ...
 e.extend(validate_authorities(state,'state ','gate_state'))
 ...
 req(selection.get('separate_d4_acceptance_required') is True and ledger.get('separate_d4_acceptance_required') is True,'D4 acceptance separation drift')
 return e
def main():
 ...
 if errors:return 1
 print('d4c_selection=PASS evidence=9_of_9 d4wide=26_of_26 terminal_c2_disposition=selected d4=scoped acceptance=separate authorities=unchanged')
 return 0
