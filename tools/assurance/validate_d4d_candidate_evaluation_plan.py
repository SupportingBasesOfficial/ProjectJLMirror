...
def validate(root):
 ...
 if d.get('source_decisions')!=EXPECTED_SOURCE_DECISIONS or d.get('candidate') != {'workload_identity_to_broker_credential_adapter': 'derived_short_lived_broker_native_credential_adapter', 'tenant_and_contract_scoped_producer_consumer_authorization': 'broker_acl_projection_adapter', 'message_protection_key_authority_and_historical_verifier_continuity': 'kms_backed_envelope_or_transport_protection_profile', 'secret_credential_payload_exclusion_and_erasure_boundary': 'reference_only_secret_authority_profile', 'trace_context_observability_only_validation_and_redaction': 'w3c_trace_context_bounded_profile'} or d.get('candidate_status') != 'selected_c2_security_profile' or d.get('state') != 'selected_candidate': e.append('D4-D state boundary drift')
...
if __name__=='__main__':
 ...
 print('d4d_candidate_evaluation_plan=PASS axes=5 selection=not_selected current_d4d=5_of_5 d4wide=26/26 authorities=unchanged')
