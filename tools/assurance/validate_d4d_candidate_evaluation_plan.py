...
def validate(root: Path) -> list[str]:
    ...
    if plan.get("evaluation_output_states") != ALLOWED_OUTPUTS:
        errors.append("evaluation outputs must remain evidence-only states")
    if plan.get("forbidden_outputs") != FORBIDDEN_OUTPUTS:
        errors.append("forbidden output inventory must block selection/readiness/authority")
    ...
    if not isinstance(invariants, list) or any(x not in invariants for x in required_fragments):
        errors.append("cross-axis authority invariants are incomplete")
    ...
    if d4c.get("candidate") is not None or d4c.get("candidate_status") != "not_selected" or d4c.get("state") != "candidate_selection_open":
        errors.append("D4-C must remain open/unselected after evidence completion")
    if d4c.get("evidence_completed") != D4C_CREDITS or d4c.get("evidence_remaining") != []:
        errors.append("D4-C evidence must remain exactly 9/9")
    ...
    if d4d.get("required_evidence") != D4D_REQUIRED or d4d.get("evidence_completed") != [] or d4d.get("evidence_remaining") != D4D_REQUIRED:
        errors.append("candidate evaluation must not credit D4-D evidence")
    if sum(len(t.get("evidence_completed", [])) for t in tracks.values()) != 21:
        errors.append("D4-wide evidence must remain exactly 21/26")
...
def main(argv: list[str]) -> int:
    ...
    if errors:
        ...
        return 1
    print("d4d_candidate_evaluation_plan=PASS axes=5 ir_d_002=preserved selection=not_selected evaluation_auto_credit=forbidden d4d=0_of_5 d4wide=21/26 authorities=unchanged")
    return 0
