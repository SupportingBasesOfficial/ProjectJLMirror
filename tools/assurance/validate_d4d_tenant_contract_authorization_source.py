...
MANIFEST=Path("implementation/d4-eventing-async/source-evidence/d4-d-tenant-contract-authorization/source-evidence-manifest.json")
...
EXPECTED_ID="tenant_and_contract_scoped_producer_consumer_authorization"
EXPECTED_DECISION="OPEN-EVT-016"
EXPECTED_AXIS="tenant_and_contract_scoped_producer_consumer_authorization"
FIRST_CREDIT="workload_identity_to_broker_credential_adapter_least_privilege"
EXPECTED_MUST={
"producer_authorization_is_contract_scoped_and_least_privilege",
"consumer_authorization_is_service_domain_and_tenant_constrained",
"broker_acl_or_policy_identity_does_not_replace_platform_tenant_authority",
"cross_tenant_produce_and_consume_attempts_fail_closed",
"authorization_revocation_or_generation_change_blocks_stale_broker_access",
"wildcard_or_broad_broker_permissions_cannot_bypass_contract_scope",
"authorization_projection_is_reconcilable_from_current_platform_authority",
}
REQUIRED=[
FIRST_CREDIT,
EXPECTED_ID,
...
def validate(root:Path)->list[str]:
    ...
    if m.get("broker_authorization_authority")!="projection_of_current_platform_authority": errors.append("broker authorization must remain a projection of current platform authority")
    ...
    if source_time.get("d4d")!="1_of_5_unselected" or source_time.get("d4wide")!="22_of_26": errors.append("source-time snapshot must remain 1/5 and 22/26")
    ...
    if d4d.get("required_evidence")!=REQUIRED: errors.append("D4-D required evidence drift")
    if d4d.get("evidence_completed")!=[FIRST_CREDIT] or d4d.get("evidence_remaining")!=REQUIRED[1:]: errors.append("current D4-D state must remain exactly 1/5 until separate promotion")
    if sum(len(t["evidence_completed"]) for t in state["tracks"])!=22: errors.append("D4-wide current state must remain 22/26")
...
if __name__=="__main__":
    ...
    if errors:
        for e in errors: print("D4D_AUTH_SOURCE_ERROR:",e,file=sys.stderr)
        raise SystemExit(1)
    print("d4d_tenant_contract_authorization_source=PASS source_snapshot=1_of_5 source_auto_credit=false current_d4d=1_of_5 d4wide=22/26 selection=not_selected authorities=unchanged")
