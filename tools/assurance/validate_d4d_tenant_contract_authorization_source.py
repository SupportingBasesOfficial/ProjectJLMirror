...
EXPECTED_ID='tenant_and_contract_scoped_producer_consumer_authorization'; EXPECTED_DECISION='OPEN-EVT-016'; EXPECTED_AXIS=EXPECTED_ID; FIRST='workload_identity_to_broker_credential_adapter_least_privilege'
...
REQUIRED=[FIRST,EXPECTED_ID,'message_protection_key_authority_and_historical_verifier_continuity','secret_credential_payload_exclusion_and_erasure_boundary','trace_context_observability_only_validation_and_redaction']
def validate(root):
 ...
 if d.get('required_evidence')!=REQUIRED or d.get('evidence_completed')!=[FIRST,EXPECTED_ID] or d.get('evidence_remaining')!=REQUIRED[2:]:e.append('current D4-D state must reflect exactly the separate second promotion')
 if sum(len(t['evidence_completed']) for t in state['tracks'])!=23:e.append('D4-wide current state must be 23/26')
...
if __name__=='__main__':
 ...
 print('d4d_tenant_contract_authorization_source=PASS source_snapshot=1_of_5 source_auto_credit=false current_d4d=2_of_5 d4wide=23/26 selection=not_selected authorities=unchanged')
