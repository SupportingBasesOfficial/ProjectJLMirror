...
EXPECTED_ID='tenant_and_contract_scoped_producer_consumer_authorization'; THIRD='message_protection_key_authority_and_historical_verifier_continuity'; FOURTH='secret_credential_payload_exclusion_and_erasure_boundary'; FIFTH='trace_context_observability_only_validation_and_redaction'; EXPECTED_DECISION='OPEN-EVT-016'; EXPECTED_AXIS=EXPECTED_ID; FIRST='workload_identity_to_broker_credential_adapter_least_privilege'
...
REQUIRED=[FIRST,EXPECTED_ID,THIRD,FOURTH,FIFTH]; CURRENT_CREDITED=REQUIRED[:4]
def validate(root):
 ...
 if sum(len(t['evidence_completed']) for t in state['tracks'])!=25:e.append('D4-wide current state must be 25/26')
...
if __name__=='__main__':
 ...
 print('d4d_tenant_contract_authorization_source=PASS source_snapshot=1_of_5 source_auto_credit=false current_d4d=4_of_5 d4wide=25/26 selection=not_selected authorities=unchanged')
