...
CURRENT_CREDITED = list(REQUIRED)
...
def validate(root: Path) -> list[str]:
    ...
    identity = {'schema_version':manifest.get('schema_version'),'gate_id':manifest.get('gate_id'),'track_id':manifest.get('track_id'),'source_decision':manifest.get('source_decision'),'evidence_id':manifest.get('evidence_id'),'mode':manifest.get('mode'),'source_base':manifest.get('source_base')}
    ...
    if identity != expected_identity: errors.append('source evidence immutable identity envelope drift')
    if set(manifest.get('must_prove', [])) != EXPECTED_MUST: errors.append('must_prove drift')
    if manifest.get('trace_context_authority') != 'observability_only_validated_bounded_and_redacted': errors.append('trace context authority drift')
    if manifest.get('current_run_auto_credit') is not False or manifest.get('ledger_credit') != []: errors.append('source evidence must remain non-promoting at source time')
    if manifest.get('candidate') is not None or manifest.get('candidate_status') != 'not_selected' or manifest.get('selection_authority') != 'not_granted': errors.append('source evidence must not select D4-D')
    if manifest.get('source_time_state', {}) != EXPECTED_SOURCE_TIME_STATE: errors.append('source-time snapshot or authority state drift')
    ...
    proofs = run_probes()
    if set(proofs) != EXPECTED_PROBES: errors.append('executed probe set drift')
...
if __name__ == '__main__':
    ...
    print('d4d_trace_context_source=PASS source_identity=exact source_snapshot=4_of_5 historical_gate=scoped source_auto_credit=false current_gate=separately_accepted current_d4d=5_of_5 d4wide=26/26 historical_selection=not_selected current_selection=selected_c2_security_profile authorities=unchanged probes=81')
