...
EXPECTED_ID='workload_identity_to_broker_credential_adapter_least_privilege'; SECOND='tenant_and_contract_scoped_producer_consumer_authorization'; EXPECTED_DECISION='OPEN-EVT-016'
...
REQUIRED=[EXPECTED_ID,SECOND,'message_protection_key_authority_and_historical_verifier_continuity','secret_credential_payload_exclusion_and_erasure_boundary','trace_context_observability_only_validation_and_redaction']
def validate(root):
 ...
 if d.get('required_evidence')!=REQUIRED or d.get('evidence_completed')!=[EXPECTED_ID,SECOND] or d.get('evidence_remaining')!=REQUIRED[2:]:e.append('current D4-D state must reflect exactly two separate promotions')
...
if __name__=='__main__':
 ...
 print('d4d_workload_identity_source=PASS source_snapshot=0_of_5 current_d4d=2_of_5 d4wide=23/26 selection=not_selected authorities=unchanged')
