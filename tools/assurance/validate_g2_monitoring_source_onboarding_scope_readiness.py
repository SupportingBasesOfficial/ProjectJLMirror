...
REQUIRED_LABEL = "jlmirror-slice:g2-monitoring-source-onboarding"
REQUIRED_HEAD_PREFIX = "impl/g2-monitoring-source-onboarding"
SCOPE_CONTEXT = "JLMIRROR / g2-monitoring-source-onboarding-implementation-scope"
READY_CONTEXT = "JLMIRROR / g2-monitoring-source-onboarding-merge-readiness"
WORKFLOW_NAME = "JLMIRROR G2 Monitoring Source Onboarding Implementation Scope"
WORKFLOW_PATH = ".github/workflows/g2-monitoring-source-onboarding-implementation-scope.yml"
...
def live_coordinates(pr: dict[str, Any], branch: dict[str, Any], *, repo: str, default_branch: str) -> tuple[str, str, str]:
    ...
    req(isinstance(head_ref, str) and head_ref.startswith(REQUIRED_HEAD_PREFIX), "implementation PR head ref is not canonical G2 prefix")
...
def evidence_contract(evidence: str, base_sha: str, head_sha: str) -> tuple[str, str, str, str]:
    if evidence == "scope":
        return SCOPE_CONTEXT, f"G2 scope PASS base={base_sha} head={head_sha}", "scope attestation", "attest"
    if evidence == "ready":
        return READY_CONTEXT, f"G2 ready PASS base={base_sha} head={head_sha}", "merge-readiness", "ready"
...
def select_trusted_evidence(statuses: list[dict[str, Any]], *, evidence: str, repo: str, server_url: str, base_sha: str, head_sha: str) -> tuple[dict[str, Any], int, str]:
    ...
    req(isinstance(target_url, str) and target_url.startswith(prefix), f"trusted G2 {label} target_url is not a canonical workflow run")
    suffix = target_url[len(prefix):]
    req(bool(re.fullmatch(r"[1-9][0-9]*", suffix)), f"trusted G2 {label} target_url run id is malformed")
...
def validate_workflow_run(run: dict[str, Any], *, run_id: int, repo: str, default_branch: str, base_sha: str) -> None:
    req(run.get("id") == run_id, "trusted G2 workflow run id drift")
    req(run.get("name") == WORKFLOW_NAME, "trusted G2 workflow name drift")
    req(run.get("path") == WORKFLOW_PATH, "trusted G2 workflow path drift")
    req(run.get("event") == "issue_comment", "trusted G2 workflow event must be issue_comment")
    req(run.get("status") == "completed" and run.get("conclusion") == "success", "trusted G2 workflow run is not completed successfully")
    req(run.get("head_branch") == default_branch, "trusted G2 workflow did not execute from the current default branch")
    req(run.get("head_sha") == base_sha, "trusted G2 workflow run is not bound to the exact current base SHA")
    repository = run.get("repository") or {}
    req(repository.get("full_name") == repo, "trusted G2 workflow repository drift")


def expected_publisher_job_name(*, pr_number: int, mode: str, base_sha: str, head_sha: str, head_ref: str) -> str:
    return f"g2-publish-final pr={pr_number} mode={mode} base={base_sha} head={head_sha} ref={head_ref}"
...
def validate_publisher_job(jobs_payload: dict[str, Any], *, pr_number: int, mode: str, base_sha: str, head_sha: str, head_ref: str) -> None:
    ...
    req(isinstance(jobs, list), "trusted G2 workflow jobs payload missing")
    ...
    req(len(matches) == 1, "trusted G2 evidence is not bound to the exact publisher job identity")
    publisher = matches[0]
    req(publisher.get("status") == "completed" and publisher.get("conclusion") == "success", "trusted G2 exact publisher job is not completed successfully")
...
def main() -> int:
    ...
    try:
        ...
        print(f"G2_SCOPE_READINESS_ERROR: {exc}", file=sys.stderr)
        return 1
    print(f"g2_scope_readiness=PASS evidence={args.evidence} base={base_sha} head={head_sha} ref={head_ref} trusted_creator={TRUSTED_CREATOR_LOGIN}:{TRUSTED_CREATOR_ID} publisher_job=exact run_id={run_id}")
    return 0
