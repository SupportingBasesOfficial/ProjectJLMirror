...
REQUIRED_LABEL = "jlmirror-slice:g4-metrics"
REQUIRED_HEAD_PREFIX = "impl/g4-metrics"
SCOPE_CONTEXT = "JLMIRROR / g4-metrics-implementation-scope"
READY_CONTEXT = "JLMIRROR / g4-metrics-merge-readiness"
WORKFLOW_NAME = "JLMIRROR G4 Metrics Implementation Scope"
WORKFLOW_PATH = ".github/workflows/g4-metrics-implementation-scope.yml"
...
def live_coordinates(pr: dict[str, Any], branch: dict[str, Any], *, repo: str, default_branch: str) -> tuple[str, str, str]:
    ...
    req(isinstance(head_ref, str) and head_ref.startswith(REQUIRED_HEAD_PREFIX), "implementation PR head ref is not canonical G4 prefix")
...
def evidence_contract(evidence: str, base_sha: str, head_sha: str) -> tuple[str, str, str, str]:
    if evidence == "scope":
        return SCOPE_CONTEXT, f"G4 scope PASS base={base_sha} head={head_sha}", "scope attestation", "attest"
    if evidence == "ready":
        return READY_CONTEXT, f"G4 ready PASS base={base_sha} head={head_sha}", "merge-readiness", "ready"
    raise AssertionError("unknown G4 trusted evidence kind")


def _trusted_status_order(row: dict[str, Any]) -> tuple[str, int]:
    created_at = row.get("created_at")
    status_id = row.get("id")
    req(isinstance(created_at, str) and created_at, "trusted G4 status created_at missing")
    req(type(status_id) is int and status_id > 0, "trusted G4 status id missing")
...
def select_trusted_evidence(statuses: list[dict[str, Any]], *, evidence: str, repo: str, server_url: str, base_sha: str, head_sha: str) -> tuple[dict[str, Any], int, str]:
    ...
    req(bool(trusted), f"no trusted G4 {label} status from canonical GitHub Actions publisher")
    ...
    req(len(same_latest) == 1, f"trusted G4 {label} latest status ordering is ambiguous")
    status = ordered[-1]
    req(status.get("state") == "success", f"latest trusted G4 {label} is not successful")
    req(status.get("description") == expected_description, f"trusted G4 {label} coordinates are stale or malformed")
    target_url = status.get("target_url")
    prefix = f"{server_url.rstrip('/')}/{repo}/actions/runs/"
    req(isinstance(target_url, str) and target_url.startswith(prefix), f"trusted G4 {label} target_url is not a canonical workflow run")
    suffix = target_url[len(prefix):]
    req(bool(re.fullmatch(r"[1-9][0-9]*", suffix)), f"trusted G4 {label} target_url run id is malformed")
...
def validate_workflow_run(run: dict[str, Any], *, run_id: int, repo: str, default_branch: str, base_sha: str) -> None:
    req(run.get("id") == run_id, "trusted G4 workflow run id drift")
    req(run.get("name") == WORKFLOW_NAME, "trusted G4 workflow name drift")
    req(run.get("path") == WORKFLOW_PATH, "trusted G4 workflow path drift")
    req(run.get("event") == "issue_comment", "trusted G4 workflow event must be issue_comment")
    req(run.get("status") == "completed" and run.get("conclusion") == "success", "trusted G4 workflow run is not completed successfully")
    req(run.get("head_branch") == default_branch, "trusted G4 workflow did not execute from the current default branch")
    req(run.get("head_sha") == base_sha, "trusted G4 workflow run is not bound to the exact current base SHA")
    repository = run.get("repository") or {}
    req(repository.get("full_name") == repo, "trusted G4 workflow repository drift")
...
def validate_publisher_job(jobs_payload: dict[str, Any], *, pr_number: int, mode: str, base_sha: str, head_sha: str, head_ref: str) -> None:
    ...
    req(isinstance(jobs, list), "trusted G4 workflow jobs payload missing")
    ...
    req(len(matches) == 1, "trusted G4 evidence is not bound to the exact publisher job identity")
    publisher = matches[0]
    req(publisher.get("status") == "completed" and publisher.get("conclusion") == "success", "trusted G4 exact publisher job is not completed successfully")
...
def main() -> int:
    ...
    try:
        ...
        print(f"G4_SCOPE_READINESS_ERROR: {exc}", file=sys.stderr)
        return 1
    print(f"g4_scope_readiness=PASS evidence={args.evidence} base={base_sha} head={head_sha} ref={head_ref} trusted_creator={TRUSTED_CREATOR_LOGIN}:{TRUSTED_CREATOR_ID} publisher_job=exact run_id={run_id}")
    return 0
