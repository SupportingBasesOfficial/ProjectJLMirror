...
MANIFEST_PATH = "implementation/g5-problem-health-authorization/AUTHORIZATION_MANIFEST.json"
...
def semantic_errors(path: str, text: str, policy: dict) -> list[str]:
    ...
    for marker in policy.get("forbidden_code_markers") or []:
        ...
        if marker_folded and marker_folded in folded:
            errors.append(f"forbidden G5 semantic marker '{marker}' in {path}")
    if SQL_MUTATION_RE.search(text):
        errors.append(f"direct SQL/schema mutation is forbidden in G5 executable artifact: {path}")
    ...
    if property_write_re.search(text) or bracket_write_re.search(text) or getattr_write_re.search(text):
        errors.append(f"direct persistence write is forbidden in G5 executable artifact: {path}")
    return errors
...
def runtime_workflow_errors(text: str, policy: dict) -> list[str]:
    ...
    try:
        ...
        return ["G5 runtime workflow must use canonical JSON-compatible workflow structure"]
    if workflow.get("name") != policy.get("runtime_workflow_name"):
        errors.append("G5 runtime workflow name drift")
    if workflow.get("permissions") != {}:
        errors.append("G5 runtime workflow must default to zero permissions")
    triggers = workflow.get("on")
    if not isinstance(triggers, dict) or set(triggers) != {"pull_request", "workflow_dispatch"}:
        errors.append("G5 runtime workflow trigger surface drift")
    jobs = workflow.get("jobs")
    if not isinstance(jobs, dict) or set(jobs) != {"g5-runtime"}:
        errors.append("G5 runtime workflow must contain exactly g5-runtime job")
        return errors
    job = jobs["g5-runtime"]
    if not isinstance(job, dict) or job.get("runs-on") not in {"ubuntu-24.04", "ubuntu-latest"}:
        errors.append("G5 runtime job runner drift")
    steps = job.get("steps") if isinstance(job, dict) else None
    if not isinstance(steps, list) or not steps:
        errors.append("G5 runtime workflow steps missing")
        return errors
    allowed_actions = set(policy.get("runtime_allowed_actions") or [])
    runs: list[str] = []
    for step in steps:
        if not isinstance(step, dict):
            errors.append("G5 runtime workflow step must be an object")
            continue
        use = step.get("uses")
        run = step.get("run")
        if use is not None:
            if use not in allowed_actions:
                errors.append(f"G5 runtime workflow uses unauthorized action: {use}")
            if set(step) != {"uses"}:
                errors.append("G5 runtime action steps may not override repository, ref, path, credentials, env, shell, or other inputs")
            if run is not None:
                errors.append("G5 runtime workflow step cannot combine uses and run")
        elif run is not None:
            if set(step) != {"run"}:
                errors.append("G5 runtime run step may contain only the canonical run command")
            if not isinstance(run, str):
                errors.append("G5 runtime workflow run step must be a string")
            else:
                runs.append(run.strip())
        else:
            errors.append("G5 runtime workflow step must contain uses or run")
    if runs != [policy.get("runtime_entrypoint")]:
        errors.append("G5 runtime workflow must execute exactly the canonical runtime entrypoint once")
    return errors
...
def main() -> int:
    ...
    if manifest.get("implementation_authority_after_merge") != "granted_for_exact_g5_problem_health_only": errors.append("base does not grant exact G5 implementation authority")
    if args.head_repo != args.base_repo: errors.append("implementation PR must use the canonical repository")
    if not args.head_ref.startswith(policy["implementation_pr_head_prefix"]): errors.append("implementation head prefix is not canonical G5 prefix")
    try:
        labels = set(json.loads(args.labels_json))
    except Exception:
        labels = set(); errors.append("labels-json is invalid")
    if policy["implementation_pr_required_label"] not in labels: errors.append("implementation PR missing required G5 label")
    ...
    for path in changed:
        ...
        if not allowed(path, policy):
            errors.append(f"path outside G5 implementation authority: {path}")
            continue
        if any(token in lower_path for token in forbidden_tokens):
            errors.append(f"forbidden G5 path token in {path}")
    ...
    if not runtime_text:
        errors.append("G5 runtime workflow missing")
    else:
    ...
    if not claim_text:
        errors.append("G5 implementation claim missing")
    else:
        try:
            claim = json.loads(claim_text)
            if set(claim) != {"schema_version","authorization_id","slice_id"}: errors.append("G5 implementation claim shape drift")
            if claim.get("schema_version") != 1 or claim.get("authorization_id") != "g5.problem-health@1" or claim.get("slice_id") != "g5.problem-health@1": errors.append("G5 implementation claim identity drift")
        except Exception:
            errors.append("G5 implementation claim is invalid JSON")
    for error in errors:
