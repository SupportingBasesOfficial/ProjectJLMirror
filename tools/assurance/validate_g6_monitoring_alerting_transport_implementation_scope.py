...
MANIFEST_PATH="implementation/g6-monitoring-alerting-transport-authorization/AUTHORIZATION_MANIFEST.json"
...
def semantic_errors(path:str,text:str,policy:dict)->list[str]:
    ...
    for marker in policy.get("forbidden_code_markers") or []:
        ...
        if marker_folded and marker_folded in folded:
            errors.append(f"forbidden G6 semantic marker '{marker}' in {path}")
    if SQL_MUTATION_RE.search(text):
        errors.append(f"direct SQL/schema mutation is forbidden in G6 application executable artifact: {path}")
    ...
    if receiver:
        ...
        if property_write_re.search(text) or bracket_write_re.search(text) or getattr_write_re.search(text):
            errors.append(f"direct persistence write is forbidden in G6 application executable artifact: {path}")
    return errors
...
def exact_sql_errors(text:str,policy:dict)->list[str]:
    ...
    for marker in required:
        if marker not in lower:
            errors.append(f"G6 exact SQL missing required composition marker: {marker}")
    forbidden_patterns=(
        r"\bcreate\s+table\b",
        r"\bcreate\s+schema\b",
        r"\balter\s+table\s+(?:monitoring|alerting)\.",
    ...
    for pattern in forbidden_patterns:
        if re.search(pattern,text,re.IGNORECASE):
            errors.append(f"G6 exact SQL contains forbidden business-state/schema mutation: {pattern}")
    for marker in (
        "alert_id","alert_transition_id","lifecycle_state","policy_id","policy_version",
        "create_alert","resolve_alert","reopen_alert","notification_intent",
        "jlmirror_ack","incident_id","ticket_id"
    ):
        if semantic_fold(marker) in semantic_fold(text):
            errors.append(f"G6 exact SQL contains forbidden G7+ marker: {marker}")
    if "system.async_outbox_message" in lower and re.search(r"\binsert\s+into\s+system\.async_outbox_message\b",text,re.IGNORECASE):
        errors.append("G6 consumer SQL may not create new outbox messages")
    if re.search(r"\binsert\s+into\s+system\.async_consumer_inbox\b",text,re.IGNORECASE) is None:
        # The consumer must visibly compose the canonical inbox, whether directly inside a guarded function or equivalent exact SQL.
        errors.append("G6 exact SQL must admit durable receipt through system.async_consumer_inbox")
    return errors
...
def runtime_workflow_errors(text:str,policy:dict)->list[str]:
    ...
    try:
        ...
        return ["G6 runtime workflow must use canonical JSON-compatible workflow structure"]
    if workflow.get("name")!=policy.get("runtime_workflow_name"):
        errors.append("G6 runtime workflow name drift")
    if workflow.get("permissions")!={}:
        errors.append("G6 runtime workflow must default to zero permissions")
    triggers=workflow.get("on")
    if not isinstance(triggers,dict) or set(triggers)!={"pull_request","workflow_dispatch"}:
        errors.append("G6 runtime workflow trigger surface drift")
    jobs=workflow.get("jobs")
    if not isinstance(jobs,dict) or set(jobs)!={"g6-runtime"}:
        errors.append("G6 runtime workflow must contain exactly g6-runtime job")
        return errors
    job=jobs["g6-runtime"]
    if not isinstance(job,dict) or job.get("runs-on") not in {"ubuntu-24.04","ubuntu-latest"}:
        errors.append("G6 runtime job runner drift")
    steps=job.get("steps") if isinstance(job,dict) else None
    if not isinstance(steps,list) or not steps:
        errors.append("G6 runtime workflow steps missing")
        return errors
    allowed_actions=set(policy.get("runtime_allowed_actions") or [])
    runs=[]
    for step in steps:
        if not isinstance(step,dict):
            errors.append("G6 runtime workflow step must be an object"); continue
        use=step.get("uses"); run=step.get("run")
        if use is not None:
            if use not in allowed_actions:
                errors.append(f"G6 runtime workflow uses unauthorized action: {use}")
            if set(step)!={"uses"}:
                errors.append("G6 runtime action steps may not override repository, ref, path, credentials, env, shell, or other inputs")
            if run is not None:
                errors.append("G6 runtime workflow step cannot combine uses and run")
        elif run is not None:
            if set(step)!={"run"}:
                errors.append("G6 runtime run step may contain only the canonical run command")
            if isinstance(run,str):
                runs.append(run.strip())
            else:
                errors.append("G6 runtime workflow run step must be a string")
        else:
            errors.append("G6 runtime workflow step must contain uses or run")
    if runs!=[policy.get("runtime_entrypoint")]:
        errors.append("G6 runtime workflow must execute exactly the canonical runtime entrypoint once")
    return errors
...
def main()->int:
    ...
    if manifest.get("implementation_authority_after_merge")!="granted_for_exact_g6_monitoring_alerting_transport_only":
        errors.append("base does not grant exact G6 implementation authority")
    if args.head_repo!=args.base_repo:
        errors.append("implementation PR must use the canonical repository")
    if not args.head_ref.startswith(policy["implementation_pr_head_prefix"]):
        errors.append("implementation head prefix is not canonical G6 prefix")
    try:
    ...
    if policy["implementation_pr_required_label"] not in labels:
        errors.append("implementation PR missing required G6 label")
    ...
    for path in changed:
        ...
        if not allowed(path,policy):
            errors.append(f"path outside G6 implementation authority: {path}")
            continue
        if path!=policy.get("exact_sql_path") and any(token in lower_path for token in forbidden_tokens):
            errors.append(f"forbidden G6 path token in {path}")
    ...
    if not sql_text:
        errors.append("G6 exact integration SQL migration missing")
    else:
    ...
    if not runtime_text:
        errors.append("G6 runtime workflow missing")
    else:
    ...
    if not claim_text:
        errors.append("G6 implementation claim missing")
    else:
        try:
            claim=json.loads(claim_text)
            if set(claim)!={"schema_version","authorization_id","slice_id"}:
                errors.append("G6 implementation claim shape drift")
            if claim.get("schema_version")!=1 or claim.get("authorization_id")!="g6.monitoring-alerting-transport@1" or claim.get("slice_id")!="g6.monitoring-alerting-transport@1":
                errors.append("G6 implementation claim identity drift")
        except Exception:
            errors.append("G6 implementation claim is invalid JSON")

    for error in errors:
        print(f"G6_SCOPE_ERROR: {error}",file=sys.stderr)
    if errors:
        return 1
    print(f"g6_scope=PASS base={args.base} head={args.head} changed_files={len(changed)}")
    return 0
