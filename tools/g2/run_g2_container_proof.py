...
IMAGE = "jlmirror-g2-runtime-proof:local"
CONTAINER = "jlmirror-g2-runtime-proof"
...
def generate_tls(temp: Path) -> tuple[Path, Path]:
    ...
    if openssl is None:
        raise AssertionError("openssl is required for the bounded container proof")
    cert = temp / "cert.pem"
    key = temp / "key.pem"
    run([
...
def wait_ready(url: str) -> str:
    for _ in range(120):
        ...
        if state.returncode == 0 and state.stdout.strip().startswith("exited|"):
            ...
            raise AssertionError(
                "G2 container exited before readiness check state="
                + state.stdout.strip()
                + "\n"
                + logs.stderr[-3000:]
                + logs.stdout[-3000:]
        ...
        time.sleep(0.1)
    logs = run(["docker", "logs", CONTAINER], check=False)
    state = run(["docker", "inspect", CONTAINER, "--format", "{{json .State}}"], check=False)
    raise AssertionError(
        "G2 container did not become ready state="
        + state.stdout[-2000:]
        + "\n"
        + logs.stderr[-3000:]
        + logs.stdout[-3000:]
...
def main() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="jlmirror-g2-container-") as tmp:
            temp = Path(tmp)
            ...
            run([
                "docker", "build",
                "--pull=false",
                "--build-arg", "BASE_IMAGE=" + digest,
                "-f", "apps/g2-monitoring-source-onboarding/Dockerfile.runtime-proof",
                "-t", IMAGE,
                ".",
            ])
            ...
            run([
                ...
                "--host", "127.0.0.1",
                "--port", str(port),
                "--certfile", "/run/jlmirror/cert.pem",
                "--keyfile", "/run/jlmirror/key.pem",
            ])

            base = "https://127.0.0.1:" + str(port)
            body = wait_ready(base + "/")
            if 'id="g2-onboarding"' not in body:
                raise AssertionError("readiness response drift")

            status, fixture_body = request(base + "/__fixture__/g2/login/start?case=success")
            if status != 404 or fixture_body != '{"state":"unavailable"}':
                raise AssertionError("container boot unexpectedly exposed fixture-only G2 surface")

            running = run([
                "docker", "inspect", CONTAINER,
                "--format", "{{.State.Running}}|{{.HostConfig.ReadonlyRootfs}}|{{json .HostConfig.CapDrop}}|{{json .HostConfig.SecurityOpt}}|{{.Config.User}}",
            ]).stdout.strip()
            ...
            print(
                "g2_container_proof=PASS "
                "base_digest=" + digest.split("@", 1)[1] + " "
                "image_id=" + image_id + " "
