...
def wait_ready(url: str) -> None:
    ...
    for _ in range(120):
        try:
            ...
            time.sleep(0.1)
    raise AssertionError("G4 BFF did not become ready")
...
def run_browser(
    ...
    *,
    ...
    if 'data-e2e-result="' + marker + '"' not in html:
        log_tail = (
            server_log.read_text(encoding="utf-8", errors="replace")[-5000:]
...
def main() -> int:
    executable = browser()
    with tempfile.TemporaryDirectory(prefix="jlmirror-g4-browser-") as tmp:
        temp = Path(tmp)
        cert = temp / "cert.pem"
        key = temp / "key.pem"
        run([
        ...
        env = dict(os.environ)
        parts = [str(ROOT / "src"), str(ROOT / "apps/g4-metrics")]
        if env.get("PYTHONPATH"):
            parts.append(env["PYTHONPATH"])
        env["PYTHONPATH"] = os.pathsep.join(parts)

        port = free_port()
        server_log = temp / "g4-bff.log"
        server_log_handle = server_log.open("w", encoding="utf-8")
        server = subprocess.Popen([
            sys.executable,
            "apps/g4-metrics/bff_server.py",
            "--host", "127.0.0.1",
            "--port", str(port),
            "--certfile", str(cert),
            "--keyfile", str(key),
        ...
        try:
            ...
            for profile_name, case_name, marker in (
                ("success", "success", "g4-success-pass"),
                ("history", "history", "g4-history-pass"),
                ("revoked", "revoked", "g4-revoked-pass"),
                ("cross", "cross-tenant", "g4-cross-tenant-pass"),
            ):
                run_browser(
                    executable,
                    profile=temp / ("profile-" + profile_name),
                    url=base + "/__fixture__/g4/login/start?case=" + case_name,
                    marker=marker,
                    server_log=server_log,
                )
            print(
                "g4_browser_e2e=PASS definitions=PASS current=PASS history=PASS "
                "revoked=PASS cross_tenant=PASS missing_not_zero=PASS"
            )
        finally:
            server.terminate()
            try:
            ...
            if server.returncode not in (0, -15):
                ...
                raise AssertionError("G4 BFF exited unexpectedly: " + logs[-3000:])
    return 0
