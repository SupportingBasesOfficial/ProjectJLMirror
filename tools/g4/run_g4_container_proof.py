...
IMAGE = "jlmirror-g4-runtime-proof:local"
CONTAINER = "jlmirror-g4-runtime-proof"
...
def wait_ready(url: str) -> str:
    for _ in range(120):
        ...
        if state.returncode == 0 and state.stdout.strip().startswith("exited|"):
            ...
            raise AssertionError(
                "G4 container exited before readiness\n" + logs.stderr[-3000:] + logs.stdout[-3000:]
            )
        try:
            ...
            pass
        time.sleep(0.1)
    raise AssertionError("G4 container did not become ready")
...
def main() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="jlmirror-g4-container-") as tmp:
            temp = Path(tmp)
            cert, key = generate_tls(temp)
            run([
                "docker", "build", "--pull=false", "--build-arg", "BASE_IMAGE=" + digest,
                "-f", "apps/g4-metrics/Dockerfile.runtime-proof",
                "-t", IMAGE, ".",
            ])
            ...
            if 'id="g4-metrics"' not in body:
                raise AssertionError("G4 readiness response drift")
            status, fixture_body = request(base + "/__fixture__/g4/login/start?case=success")
            if status != 404 or fixture_body != '{"state":"unavailable"}':
                raise AssertionError("container boot unexpectedly exposed fixture-only G4 surface")
            ...
            print(
                "g4_container_proof=PASS base_digest=" + digest.split("@", 1)[1]
                + " image_id=" + image_id
