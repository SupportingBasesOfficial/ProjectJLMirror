...
IMAGE="jlmirror-g5-runtime-proof:local"
CONTAINER="jlmirror-g5-runtime-proof"
...
def wait_ready(url:str)->str:
    for _ in range(120):
        ...
        if state.returncode==0 and state.stdout.strip().startswith("exited|"):
            ...
            raise AssertionError("G5 container exited before readiness\n"+logs.stderr[-3000:]+logs.stdout[-3000:])
        try:
            ...
            pass
        time.sleep(0.1)
    raise AssertionError("G5 container did not become ready")
...
def main()->int:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="jlmirror-g5-container-") as tmp:
            temp=Path(tmp)
            cert,key=generate_tls(temp)
            run([
                "docker","build","--pull=false","--build-arg","BASE_IMAGE="+digest,
                "-f","apps/g5-problem-health/Dockerfile.runtime-proof",
                "-t",IMAGE,".",
            ])
            ...
            if 'id="g5-problem-health"' not in body:
                raise AssertionError("G5 readiness response drift")
            status,fixture_body=request(base+"/__fixture__/g5/login/start?case=active")
            if status!=404 or fixture_body!='{"state":"unavailable"}':
                raise AssertionError("container boot unexpectedly exposed fixture-only G5 surface")
            ...
            if not running.endswith("|"+expected_user):
                raise AssertionError("container did not run as bounded non-root proof user")

            print(
                "g5_container_proof=PASS base_digest="+digest.split("@",1)[1]
                +" image_id="+image_id
