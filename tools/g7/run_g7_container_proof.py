...
IMAGE="jlmirror-g7-runtime-proof:local"
CONTAINER="jlmirror-g7-runtime-proof"
...
def resolve_digest()->str:
    ...
    if len(matches)!=1:
        raise AssertionError("expected exactly one immutable Python RepoDigest")
    return matches[0]
...
def main()->int:
    ...
    try:
        run([
            ...
            "-f","apps/g7-alert-policy-lifecycle/Dockerfile.runtime-proof",
            "-t",IMAGE,".",
        ])
        image_id=run(["docker","image","inspect",IMAGE,"--format","{{.Id}}"]).stdout.strip()
        if not image_id.startswith("sha256:"):
            raise AssertionError("G7 runtime image is not content-addressed")
        ...
        if "g7_worker_boot=PASS" not in cp.stdout:
            raise AssertionError("G7 worker self-check drift")
        ...
        if not inspect.startswith("true|") or "ALL" not in inspect or "no-new-privileges" not in inspect:
            raise AssertionError("G7 container hardening drift")
        if "|"+uid+"|" not in inspect or not inspect.endswith("|none"):
            raise AssertionError("G7 container user/network hardening drift")

        print(
            "g7_container_proof=PASS base_digest="+digest.split("@",1)[1]
            +" image_id="+image_id
            +" readonly=PASS cap_drop=ALL no_new_privileges=PASS network=none non_root="+uid
        )
        return 0
    finally:
        subprocess.run(["docker","rm","-f",CONTAINER],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
