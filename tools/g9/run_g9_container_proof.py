...
IMAGE="jlmirror-g9-runtime-proof:local"
CONTAINER="jlmirror-g9-runtime-proof"
...
def main()->int:
    ...
    try:
        run([
            ...
            "-f","apps/g9-notification-delivery/Dockerfile.runtime-proof",
            "-t",IMAGE,".",
        ])
        uid="65532:65532"
        cp=run([
        ...
        if "g9_worker_boot=PASS" not in cp.stdout:
            raise AssertionError("G9 worker self-check drift")
        inspect=run([
        ...
        if not inspect.startswith("true|") or "ALL" not in inspect or "no-new-privileges" not in inspect:
            raise AssertionError("G9 container hardening drift")
        if "|"+uid+"|" not in inspect or not inspect.endswith("|none"):
            raise AssertionError("G9 container user/network drift")
        print("g9_container_proof=PASS readonly=PASS cap_drop=ALL no_new_privileges=PASS network=none non_root="+uid)
        return 0
    finally:
        subprocess.run(["docker","rm","-f",CONTAINER],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
        subprocess.run(["docker","image","rm","-f",IMAGE],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
