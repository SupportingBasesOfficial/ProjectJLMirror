...
for migration in \
  ...
  sql/wave4/014_zabbix_host_inventory_resource_epoch_insert.sql; do
...
# Bootstrap source creation establishes volatile runtime admission for the live deployment.
admission_epoch="$(docker exec "$PG_CONTAINER" psql -Atq -U postgres -d "$PG_DATABASE" -c "BEGIN; SET LOCAL ROLE wave4_runtime; SET LOCAL jlmirror.tenant_id='tenant-a'; SELECT host_inventory_poll_epoch FROM monitoring.monitoring_host_inventory_runtime_admission WHERE monitoring_source_id='source-recovery'; COMMIT;" | tail -n1)"
test "$admission_epoch" = "1"
...
# Same-tenant runtime writers cannot advance the poll fence directly anymore.
...
if docker exec "$PG_CONTAINER" psql -q -v ON_ERROR_STOP=1 -U postgres -d "$PG_DATABASE" -c "BEGIN; SET LOCAL ROLE wave4_runtime; SET LOCAL jlmirror.tenant_id='tenant-a'; SELECT monitoring.reestablish_zabbix_host_inventory_poll_epoch('tenant-a','source-recovery',1,2,'placement-v2','recovery-v2','recovery-admission-v2'); COMMIT;" >/tmp/wave4-runtime-readmit.out 2>&1; then
...
docker exec "$PG_CONTAINER" psql -q -v ON_ERROR_STOP=1 -U postgres -d "$PG_DATABASE" -c "BEGIN; SET LOCAL jlmirror.tenant_id='tenant-a'; SELECT monitoring.reestablish_zabbix_host_inventory_poll_epoch('tenant-a','source-recovery',1,2,'placement-v2','recovery-v2','recovery-admission-v2'); COMMIT;" >/dev/null
...
# A pre-recovery running poll cannot publish in the successor epoch even if its local generation still matches.
...
grep -F "Monitoring resource authority fields require guarded host-inventory executor" /tmp/wave4-presence-launder.out >/dev/null
...
grep -F "Monitoring resource authority fields require guarded host-inventory executor" /tmp/wave4-scope-mutation.out >/dev/null

echo "wave4_zabbix_host_inventory_recovery_authority=PASS poll_epoch=recovery-safe runtime_forward_increment=blocked presence_evidence_laundering=blocked scope_state_direct_mutation=blocked executor=dedicated-nobypassrls"
