...
WORK_AUTHORITY = ROOT / "sql/wave4/019_zabbix_host_inventory_work_identity_and_invocation_authority.sql"
...
def main() -> None:
    for path in (
        ...
        EXPLICIT_AUTHORITY, SUPERSEDED_EPOCH, CLAIM_REVISION, WORK_AUTHORITY,
        TEST, PG, PG_ORDERING, PG_AUTHORITY, PG_FINAL_SCHEMA, PG_SUPERSEDED_EPOCH,
    ...
    work_authority = WORK_AUTHORITY.read_text(encoding="utf-8")
    ...
    for marker in (
        "MAX_HOSTS_PER_SNAPSHOT = 50_000", "class ZabbixHostEvidence", "def canonical_evidence",
        "def evidence_fingerprint", "class ZabbixHostSnapshot", "snapshot.complete",
        "InventoryFailureClass.SNAPSHOT_TRUNCATED", "InventoryFailureClass.SCOPE_EVIDENCE_INVALID",
        "InventoryFailureClass.SCOPE_ANCHOR_INACCESSIBLE", "def hostgroup_get",
        "visible_groups = tuple(host_reader.hostgroup_get", "opaque_token(\"mon-host-claim\")",
        "persisted_result = self._repository.complete_host_inventory", "return persisted_result",
    ...
    for marker in (
        "CREATE TABLE monitoring.monitoring_resource", "resource_kind TEXT NOT NULL CHECK (resource_kind = 'host')",
        ...
        "CREATE TABLE monitoring.monitoring_resource_provider_evidence", "octet_length(normalized_evidence::text) <= 65536",
        ...
        "CREATE FUNCTION monitoring.claim_zabbix_host_inventory", "CREATE FUNCTION monitoring.complete_zabbix_host_inventory",
        "FOR UPDATE OF s", "IF p_snapshot_complete THEN", "presence_state='removed'", "execution.stale_authority",
    ):
    ...
    for marker in (
        ...
        "monitoring_resource_provider_evidence_bounded_shape", "wave4_guard_host_provider_evidence_insert",
        "host provider evidence lacks configured-scope membership", "monitoring_source_validation_evidence",
        "monitoring.host_inventory_source_not_validated_current", "monitoring.host_inventory_stale_authority",
    ...
    for marker in (
        "monitoring_resource_latest_provider_evidence_owner_fk", "monitoring_sync_operation_host_inventory_snapshot_owner_fk",
        "wave4_guard_monitoring_resource_update", "Monitoring resource canonical/provider identity is immutable",
    ...
    for marker in (
        "host_inventory_poll_generation BIGINT NOT NULL DEFAULT 0", "last_confirmed_present_poll_generation BIGINT NOT NULL DEFAULT 0",
        "removed_poll_generation BIGINT NULL", "CREATE OR REPLACE FUNCTION monitoring.complete_zabbix_host_inventory",
        "host_inventory_poll_generation=s.host_inventory_poll_generation+1", "FOR UPDATE OF s,o",
        "execution.superseded_poll_authority", "last_confirmed_present_poll_generation=v_poll_generation",
        "removed_poll_generation=v_poll_generation", "e.host_inventory_poll_generation > OLD.last_confirmed_present_poll_generation",
        "newer complete authoritative negative poll evidence", "Poll generation, not wall-clock time, is the authoritative presence ordering field",
    ...
    for marker in (
        "wave4_guard_host_inventory_snapshot_insert", "not bound to its claimed running operation",
        "wave4_guard_host_provider_evidence_authority", "open claimed snapshot operation", "wave4_python_canonical_jsonb",
        "fingerprint does not match normalized evidence", "wave4_deferred_monitoring_resource_presence_authority",
        "positive presence requires accepted provider snapshot authority", "completed newer authoritative negative poll evidence",
    ...
    for marker in (
        "wave4_guard_monitoring_source_poll_generation_update", "poll generation cannot rewind",
        "wave4_guard_sync_operation_poll_generation_update", "operation poll generation is immutable after claim",
        "s.host_inventory_poll_generation=NEW.host_inventory_poll_generation", "requires current claimed poll authority",
        "wave4_deferred_host_inventory_snapshot_closure", "host_count must equal final provider evidence membership",
    ...
    for marker in (
        ...
        "last_observed_at IS DISTINCT FROM OLD.last_observed_at", "last_confirmed_present_at IS DISTINCT FROM OLD.last_confirmed_present_at",
        "removed_at IS DISTINCT FROM OLD.removed_at", "Monitoring resource authority fields require guarded host-inventory executor",
    ):
        require(marker in explicit_authority, f"explicit/final resource authority guard missing: {marker}")

    for marker in ("execution.superseded_poll_epoch_authority", "v_poll_epoch IS NULL OR v_poll_epoch <> v_current_poll_epoch", "host_inventory_poll_epoch=v_current_poll_epoch"):
        require(marker in superseded_epoch, f"superseded epoch retirement guard missing: {marker}")

    for marker in (
        "wave4_guard_host_inventory_claim_revision_update", "NEW.monitoring_source_id IS DISTINCT FROM OLD.monitoring_source_id",
        "NEW.source_instance_generation IS DISTINCT FROM OLD.source_instance_generation",
        "NEW.configuration_revision IS DISTINCT FROM OLD.configuration_revision", "NEW.scope_revision IS DISTINCT FROM OLD.scope_revision",
    ):
        require(marker in claim_revision, f"claimed host inventory revision authority guard missing: {marker}")

    for marker in (
        "jlmirror_wave4_host_inventory_invoker", "Host inventory work creation requires guarded executor authority",
        ...
        "REVOKE EXECUTE ON FUNCTION monitoring.complete_zabbix_host_inventory", "FROM PUBLIC",
        "TO jlmirror_wave4_host_inventory_invoker", "NEW.started_at IS NOT NULL", "NEW.completed_at IS NOT NULL",
        "NEW.attempt_count IS DISTINCT FROM 0",
    ):
        require(marker in work_authority, f"work/invocation authority hardening missing: {marker}")
    ...
    for marker in ("poll_generation=claim_ordered", "late_completion=retired_without_mutation", "stale_negative=blocked", "newer_positive=preserved", "unfenced_helper=absent", "poll_generation=presence_authority"):
        require(marker in ordering_pg, f"host inventory ordering PostgreSQL falsifier missing: {marker}")
    for marker in (
        "sql/wave4/009_zabbix_host_inventory_evidence_authority_hardening.sql", "sql/wave4/010_zabbix_host_inventory_final_authority_hardening.sql",
        "snapshot_operation_binding=closed", "membership_after_completion=closed", "positive_transition=evidence_bound",
        "fingerprint=verified", "negative_transition=completed_operation_bound", "poll_rewind=blocked",
        "superseded_snapshot=current-poll-bound", "host_count=final-membership-verified",
    ):
    ...
    for migration in (
        "001_monitoring_source_foundation.sql", "008_zabbix_host_inventory_poll_ordering_hardening.sql",
        "009_zabbix_host_inventory_evidence_authority_hardening.sql", "010_zabbix_host_inventory_final_authority_hardening.sql",
        "011_zabbix_host_inventory_terminal_operation_hardening.sql", "011a_zabbix_host_inventory_executor_roles.sql",
        "012_zabbix_host_inventory_recovery_authority_hardening.sql", "013_zabbix_host_inventory_executor_privileges.sql",
        "014_zabbix_host_inventory_resource_epoch_insert.sql", "015_zabbix_host_inventory_explicit_admission_and_resource_authority.sql",
        "016_zabbix_host_inventory_operation_insert_authority.sql", "017_zabbix_host_inventory_superseded_epoch_retirement.sql",
        "018_zabbix_host_inventory_claim_revision_immutability.sql", "019_zabbix_host_inventory_work_identity_and_invocation_authority.sql",
    ):
        require(migration in final_schema_pg, f"final-schema conformance omits migration: {migration}")
    for marker in (
        "schema=001-019", "complete_snapshot=accepted", "negative_removal=accepted", "superseded_poll=retired",
        "terminal_reopen=blocked", "direct_work_creation=blocked", "pending_retarget=blocked",
        "claimed_revision_authority=immutable", "invocation_authority=provider-worker-only", "invoker_table_dml=none",
        "poll_authority=single-owner-index", "observation_forgery=blocked", "logical_recovery_admission=excluded",
        "pg_dump_recovery_safe.sh", "recovered_admission_count",
    ):
        require(marker in final_schema_pg, f"final-schema PostgreSQL proof missing: {marker}")
    for marker in ("schema=001-019", "old_epoch=retired", "reconciliation=required", "stale_snapshot=absent", "invocation_authority=provider-worker-only"):
    ...
    for surface in (
        "sql/wave4/006_zabbix_host_inventory_boundary_hardening.sql", "sql/wave4/007_zabbix_host_inventory_integrity_hardening.sql",
        "sql/wave4/008_zabbix_host_inventory_poll_ordering_hardening.sql", "sql/wave4/009_zabbix_host_inventory_evidence_authority_hardening.sql",
        "sql/wave4/010_zabbix_host_inventory_final_authority_hardening.sql", "sql/wave4/015_zabbix_host_inventory_explicit_admission_and_resource_authority.sql",
        "sql/wave4/016_zabbix_host_inventory_operation_insert_authority.sql", "sql/wave4/017_zabbix_host_inventory_superseded_epoch_retirement.sql",
        "sql/wave4/018_zabbix_host_inventory_claim_revision_immutability.sql", "sql/wave4/019_zabbix_host_inventory_work_identity_and_invocation_authority.sql",
    ):
        require(surface in manifest["code_surfaces"], f"manifest missing hardening surface: {surface}")
    for surface in (
        "tools/wave4/pg_dump_recovery_safe.sh", "tools/wave4/run_zabbix_host_inventory_final_schema_postgres_conformance.sh",
        "tools/wave4/run_zabbix_host_inventory_superseded_epoch_postgres_conformance.sh", "tools/wave4/validate_zabbix_host_inventory_invocation_authority.py",
    ):
        require(surface in manifest["assurance_surfaces"], f"manifest missing assurance/recovery surface: {surface}")
    for capability in (
        "host_inventory_ingestion", "host_inventory_poll_generation_fence", "stale_negative_snapshot_rejection",
        "host_inventory_snapshot_operation_binding", "host_inventory_closed_snapshot_membership", "host_inventory_positive_presence_authority",
        "host_inventory_evidence_fingerprint_verification", "host_inventory_scope_anchor_revalidation", "host_inventory_poll_generation_rewind_rejection",
        "host_inventory_current_poll_snapshot_binding", "host_inventory_snapshot_cardinality_closure", "host_inventory_explicit_runtime_admission",
        "host_inventory_resource_observation_authority", "host_inventory_logical_recovery_admission_exclusion",
        "host_inventory_final_schema_composed_conformance", "host_inventory_superseded_epoch_retirement",
        "host_inventory_persisted_completion_outcome", "host_inventory_claim_revision_authority_immutability",
        "host_inventory_work_identity_immutable_from_enqueue", "host_inventory_provider_worker_invocation_authority",
    ...
    require("worker.provider-integration@1" in manifest["runtime_profiles"], "manifest must declare concrete provider-integration worker specialization")
    ...
    print("wave4_zabbix_host_inventory_validation=PASS resource_kind=host provider_object_kind=zabbix_host evidence=bounded+scope-bound+owner-bound+operation-bound+closed-membership+cardinality-bound+fingerprint-verified identity=immutable presence=positive+negative-authority-bound tenant_rls=forced stale_authority=fenced poll_generation=single-winner+nonrewind snapshot=current-poll-bound scope_anchor=same-cycle-revalidated recovery=explicit-admission+logical-dump-excluded+superseded-epoch-retired work_identity=enqueue-frozen invocation=provider-worker-only completion=post-persistence-authoritative final_schema=001-019-composed")
